...
class FollowerController(Node):
    def __init__(self):
        ...
        # robot_state: 'FOLLOWING' | 'RELAY_CANDIDATE' | 'STATIONARY_RELAY'
        self.robot_state = 'FOLLOWING'
        self.relay_threshold = 0.85          # comm_range * 0.85 = 1.7m
        self.last_relay_pos = None           # 마지막 relay(or 시작점) 위치
        self.relay_positions = []            # 배치된 relay 좌표 목록
        self._relay_logged = False           # RELAY_PLACED 중복 출력 방지
        ...
        self.k_angular = 2.5
        self.max_linear = 0.35            # burger 정격 속도 수준으로 복구
    ...
    def control_loop(self):
        ...
        if not self._init_done:
            ...
            if self._init_ticks >= 10:
                ...
                # 시작 위치를 첫 번째 relay 기준점으로 설정
                self.last_relay_pos = (
                    self.follower_pose.position.x,
                    self.follower_pose.position.y
                )
                self.get_logger().info(
                    f'odom stabilized, starting control. '
        ...
        # ── Relay Placement 판단 ──────────────────────────────────────────
        # STATIONARY_RELAY 상태면 완전 정지 (이후 모든 이동 로직 건너뜀)
        # Phase 4(multi-follower)에서는 여기서 "역할 전환" 신호를 follower2에 전달
        # 현재(1대): relay 배치 개념 검증 → 정지로 종료
        ...
        # last_relay_pos 기준 거리 계산
        if self.last_relay_pos is not None:
            rlx, rly = self.last_relay_pos
            dist_from_relay = math.hypot(fx - rlx, fy - rly)
            relay_trigger_dist = self.comm_range * self.relay_threshold

            if self.robot_state == 'FOLLOWING':
                if dist_from_relay >= relay_trigger_dist * 0.90:   # 90% 도달 → 후보 예고
                    self.robot_state = 'RELAY_CANDIDATE'
                    self.get_logger().warn(
                        f'RELAY_CANDIDATE: dist_from_last_relay='
                        f'{dist_from_relay:.2f}m '
                        f'(trigger={relay_trigger_dist:.2f}m)'
                    )

            if self.robot_state == 'RELAY_CANDIDATE':
                self.get_logger().info(
                    f'RELAY_CANDIDATE: dist={dist_from_relay:.2f}m / '
                    f'{relay_trigger_dist:.2f}m',
                    throttle_duration_sec=0.5
                )
                if dist_from_relay >= relay_trigger_dist:
                    # ── Phase 3: 실제 relay 배치 및 정지 ──────────────────
                    self.robot_state = 'STATIONARY_RELAY'
                    self.relay_positions.append((fx, fy))
                    self.last_relay_pos = (fx, fy)
                    self._relay_logged = True
                    self.get_logger().warn(
                        f'RELAY_PLACED: id=follower1, '
                        f'x={fx:.3f}, y={fy:.3f}, '
        ...
        if not self.use_breadcrumb:
            ...
            self.get_logger().info(
                f'[DIRECT] state={self.robot_state}, d={distance:.2f}, '
                f'e_theta={e_theta:.2f}, '
                f'v={cmd.linear.x:.2f}, w={cmd.angular.z:.2f}',
                throttle_duration_sec=1.0
            )
        ...
        self.get_logger().info(
            f'[BREAD] state={self.robot_state}, '
