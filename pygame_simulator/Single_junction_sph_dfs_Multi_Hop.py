...
# The root group (parent_group_id is None) never receives the physical
# barrier/gatekeeping repulsion (compute_khop_barrier_segment_force only
# applies to khop_branch_groups(), i.e. groups *with* a parent_group_id) --
...
def khop_estimate_corridor_width(
    ...
    """Estimate width from two robot-local wall rays, then lock it.

    Previously blended toward each new measurement at a slow 20%-per-refresh
    rate, which both took many refresh cycles to approach even its own
    (already conservative) asymptote and kept quietly drifting for as long
    as a group stayed FORMING/EXPLORING -- a physical Shepherd line that
    keeps resizing itself doesn't read as one fixed barrier. Once the first
    two-sided wall reading comes back valid, take it directly (no blending)
    and lock the group's width for the rest of its lifetime: the cap forms
    to its real measured span immediately instead of over dozens of cycles,
    and never dynamically changes afterward.
    """
    fallback = COMM_SAFE_DISTANCE * 1.85
    if group is None:
        return fallback
    if group.width_locked:
    ...
    if left < maximum - KHOP_LOCAL_PROBE_STEP and right < maximum - KHOP_LOCAL_PROBE_STEP:
        ...
        group.estimated_width = measured
        group.width_locked = True
    return group.estimated_width or fallback
...
def split_khop_from_clusters(
    ...
    for index, cluster in enumerate(clusters):
        ...
        # cluster.heading (passed in as `heading`) is a k-means mean of
        # scattered pre-filter witness headings for a *turning* cohort --
        # individual witnesses can each differ from the true branch axis by
        # up to NATURAL_SPREAD_COHORT_MAX_ANGLE_STD (25 degrees), and that
        # skew was silently driving every downstream physical measurement
        # that treats group.heading as the branch's forward axis: corridor-
        # width wall probes (khop_estimate_corridor_width), cap slot/
        # barrier-line orientation (khop_cap_slot_offsets), and the dead-end
        # forward-clearance probe -- rendering visibly diagonal, undersized
        # gate lines instead of a straight line across the real corridor.
        # The elected branch Leader's actual physical position relative to
        # the split origin is a much cleaner directional signal for a
        # *turning* branch (it already passed Max-Min d-cluster election and
        # connectivity filtering, not just the raw pre-filter observation),
        # so prefer it once it's non-trivial.
        #
        # The straight-through candidate must be excluded (direction_variance
        # == 0.0 identifies it, see detect_khop_straight_continuation_cluster):
        # its heading is exact by construction (root.heading), but its
        # cluster_robot_ids are the root cap's own members, held nearly
        # stationary by CONTROLLED_SPREAD -- their tiny in-place slot-holding
        # displacement from origin is positional *noise*, not a corridor
        # direction signal, and a first attempt at this fix that didn't
        # special-case it visibly regressed T_up's cap convergence (mean_error
        # 0.0 -> 4.9, members 34 -> 20) by replacing an already-exact heading
        # with a noisy one.
        if cluster.direction_variance > EPSILON:
            leader_robot = khop_state.robot_by_id.get(primary_head)
            if leader_robot is not None:
                displacement = leader_robot.position - origin
                # Guarded by a minimum displacement so a Leader still near
                # the origin (displacement dominated by positional noise)
                # falls back to the original cluster heading instead of
                # normalizing a near-zero-length vector.
                if displacement.length_squared() > (ROBOT_RADIUS * 2.0) ** 2:
                    group.heading = displacement.normalize()
        group.cap_anchor = (
            origin + group.heading * KHOP_BRANCH_STAGING_OFFSET
        )
...
def compute_khop_unassigned_follow_force(robot: "Robot") -> pygame.Vector2:
    ...
    if khop_state.stage in {"ADVANCING", "CONTROLLED_SPREAD"}:
        ...
        if active is not None:
            # A raw constant push along active.heading only works for a
            # robot that already has a clear line to the active branch's
            # mouth. One still physically inside a *different* branch
            # corridor just gets pinned against that corridor's own side
            # wall instead of actually reaching the active one -- this was
            # what left the general body visibly scattered across every
            # corridor instead of funneling into whichever branch is
            # actually being explored. Route through the Junction like a
            # return-to-base robot would, using the branch's display label
            # (previously display-only, "control never queries its rect")
            # as the coordinate-routing target -- the same category of
            # fixed-geometry steering assist already used for
            # RETURNING_TO_BASE, not a change to any discovery decision.
            routed = direction_toward_branch_path(
                robot.position,
                active.display_branch,
            )
            heading = (
                routed
                if routed.length_squared() > EPSILON
                else active.heading
            )
...
def direction_toward_branch_path(
    position: pygame.Vector2,
    target_branch: str,
) -> pygame.Vector2:
    """Piecewise free-space direction from anywhere to a named branch mouth.

    Mirrors direction_toward_base_path's region-hop pattern (already the
    accepted way this K-hop pipeline lets general-body *steering* consult
    fixed geometry, distinct from the strictly local-evidence-only
    Junction/branch *discovery* decisions elsewhere in the file) instead of
    a raw constant-direction bias. compute_khop_unassigned_follow_force
    used to just push every NORMAL robot toward the active branch's
    heading regardless of where it physically was -- a robot already
    inside a *different* branch corridor can't reach the active one that
    way, it just gets pinned against its own corridor's side wall. Route
    it back through that corridor's entrance and the Junction center
    first, same as a return-to-base robot would be.

    Returns a zero vector once the robot is already inside target_branch's
    own region -- the caller should fall back to the branch's own heading
    to keep advancing further into it from there.
    """
    region = get_robot_region(position)
    if region == target_branch:
        return pygame.Vector2()
    if region == "JUNCTION":
        return normalized_direction_toward(
            position,
            get_branch_entrance(target_branch),
        )
    entrance = get_region_entrance(region)
    if entrance is not None:
        return normalized_direction_toward(position, entrance)
    return pygame.Vector2()
