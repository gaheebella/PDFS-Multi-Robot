...
INITIAL_IMPULSE_POLICY_VERSION = "FAST_COMPRESSION_RELEASE_V6"
...
def compute_pressure_coupled_edf_force(
    ...
    if (
        ...
        # Keep the stored-compression release governed by its dedicated,
        # shock-limited controller.  Once that release has finished, raise
        # only the sustained branch-cruise component so the fluid body does
        # not stall halfway down a branch.
        exploration_cruise_active = not initial_pressure_release_active()
        phase_multiplier = (
            EXPLORATION_CRUISE_EDF_MULTIPLIER
            if exploration_cruise_active
            else 1.0
        )
        force_limit = (
            EXPLORATION_EDF_FORCE_LIMIT
            if exploration_cruise_active
            else EDF_PRESSURE_FORCE_LIMIT
        )
    elif (
        phase in {
            SimulationPhase.PRESSURE_PUSH,
...
def initial_pressure_release_active() -> bool:
    """Keep shock-absorbing limits active after the phase enters EXPLORE."""
    return (
        phase in {
            SimulationPhase.MOVE_TO_JUNCTION,
            SimulationPhase.EXPLORE_BRANCH,
        }
        and BASE_COMPRESSION_DURATION
        <= simulation_time
