...
NOISE_MODELS = ("none", "bounded", "gaussian")
...
EXPERIMENT_NAME = "EXP-043 W–tau Noise-Aware LiDAR Junction Detection"
DEFAULT_OUTPUT = ROOT / "lidar_junction_detection_w_tau_threshold_output"
FROZEN_PARAMETERS = {
    ...
    def __post_init__(self) -> None:
        ...
        if self.noise_model == "gaussian":
            if self.noise_sigma is None or self.noise_sigma <= 0.0:
                raise ValueError(
                    "gaussian noise requires a positive noise_sigma"
                )
...
def _inject_measurement_noise(
    ...
    elif config.noise_model == "bounded":
        noise = rng.uniform(-config.tau, config.tau, size=true_ranges.shape)
    else:
        # Gaussian samples are not strictly bounded by tau.
        noise = rng.normal(
            0.0,
            float(config.noise_sigma),
            size=true_ranges.shape,
        )
    measured = np.clip(
...
class AdaptiveSession:
    """Run unchanged physics; noise and both detectors are read-only consumers."""
    ...
    def advance_physics_frame(self) -> AdaptiveSnapshot | None:
        ...
        snapshot = AdaptiveSnapshot(
            ...
            junction_detected=active_junction,
        ...
        if active_junction and self.first_detection_frame is None:
...
class AdaptiveRenderer(BaseRenderer):
    ...
    def draw(self, session: AdaptiveSession, paused: bool) -> None:
        ...
        self.text(
            f"Moving LiDAR Junction Detection — {detector_title}",
            (18, 15),
        ...
        lines = [
            ...
            f"safe lower = W + tau = {snapshot.w_tau_lower_bound:.3f}",
            f"safe upper = Rmax - tau = {snapshot.w_tau_upper_bound:.3f}",
            f"interval valid = {snapshot.threshold_interval_valid}",
            f"alpha={config.threshold_alpha:.3f} selected T={selected_text}",
            f"smoothing window = {config.smoothing_window}",
            f"OPEN support={int(np.count_nonzero(snapshot.open_support_mask))}",
            f"opening groups={len(snapshot.opening_groups)} present={snapshot.opening_present}",
...
def write_reports(
    ...
    validation_rows = _detector_validation_rows(sessions)
    _write(
        output / "validation_table.csv",
        validation_rows,
        (
            "case", "noise", "detector", "first_detection_frame",
            "max_openings", "false_positive", "threshold",
        ),
    )
...
def _print_validation_tables(sessions: list[AdaptiveSession]) -> None:
    print(
        "case | noise | detector | first detection frame | "
        "max openings | false positive | threshold"
    )
    for row in _detector_validation_rows(sessions):
        print(
            f"{row['case']} | {row['noise']} | {row['detector']} | "
            f"{row['first_detection_frame']} | {row['max_openings']} | "
            f"{row['false_positive']} | {row['threshold']}"
        )
    config = sessions[0].config
    lower, upper, valid = config.safe_interval
    print("W | tau | lower | upper | valid | selected T")
    print(
        f"{config.worst_wall_range} | {config.tau} | {lower} | "
        f"{upper} | {valid} | {config.selected_w_tau_threshold}"
    )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--validate",
        action="store_true",
        help="A-D none/bounded validation plus deterministic replay",
    )
