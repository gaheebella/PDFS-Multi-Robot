...
2. One elected Junction Anchor stores and retransmits the NORMAL consensus;
   it never chooses a branch or assigns roles by itself.
...
7. Fixed Base-rooted LOS communication with a reactive multi-hop tail
   Breadcrumb chain.
...
FLUID_BODY_POLICY_VERSION = "CONTINUOUS_SPH_BODY_680_LARGE_MAP_V2"
ROBOT_COUNT = 680
...
ANCHOR_POLICY_VERSION = "NORMAL_CONSENSUS_MEMORY_RELAY_V1"
...
JUNCTION_TAIL_EQUILIBRIUM_SCALE = 1.15
...
# Reactive multi-hop Breadcrumb chain.  No count or slot is planned in
# advance.  Whenever the moving NORMAL tail becomes too far from the latest
# Breadcrumb, another tail NORMAL stops in place.  The same test is repeated
# as exploration advances, so the chain grows to any required number.
...
BREADCRUMB_GUARD_POLICY_VERSION = "REACTIVE_MULTI_HOP_TAIL_CHAIN_V2"
...
def initialize_relay_plan(branch):
    ...
    relay_motion_scale = 1.0
    print(
        f"[Breadcrumb] reactive multi-hop chain enabled "
        f"for branch={branch}; preplanned=0"
    )
...
def deploy_next_reactive_breadcrumb(robots) -> bool:
    """Append one relay when the latest chain node is too far from the tail.

    Returning True does not finish the chain.  This function is called again
    after the short cooldown, allowing an arbitrary number of Breadcrumbs to
    be added as the swarm advances.
    """
    if base_station is None:
        return False
    ...
    tail_gap = tail_progress - last_progress
    if tail_gap < BREADCRUMB_SPACING:
        return False
    ...
    tail_robot.relay_index = max(
        (
            breadcrumb.relay_index
            for breadcrumb in breadcrumbs
        ),
        default=-1,
    ) + 1
    link_distance = tail_robot.position.distance_to(last_node.position)
    relay_slots.append({
    ...
    print(
        f"[Breadcrumb] appended robot={tail_robot.robot_id}, "
        f"chain_count={len(breadcrumbs) + 1}, "
        f"index={tail_robot.relay_index}, "
        f"tail_gap={tail_gap:.1f}, link={link_distance:.1f}"
    )
    return True
...
def update_relay_deployment(robots, dt):
    ...
    if (
        ...
        return
    if relay_deploy_cooldown > 0.0:
        return

    if deploy_next_reactive_breadcrumb(robots):
        # At most one new relay per cooldown.  After the swarm moves farther,
        # the same live-gap test appends the next relay, with no count limit.
...
def compute_pressure_coupled_edf_force(
    ...
    if (
        ...
        phase_multiplier = TRANSFER_EDF_FORCE_MULTIPLIER
    ...
    return (
        geodesic_edf_direction(robot.position, target_branch)
        * magnitude
    )
...
def compute_route_force(robot):
    ...
    if phase == SimulationPhase.MOVE_TO_JUNCTION:
        ...
        if (
            ...
            fill_direction = geodesic_edf_direction(
                robot.position,
                active_branch,
...
while running:
    ...
    hud_lines = [
        ...
        (
            f"Reactive multi-hop Breadcrumbs="
            f"{len(get_active_branch_relays(robots))} | "
            f"next trigger gap={BREADCRUMB_SPACING:.1f} | "
            "preplanned=0"
        ),
