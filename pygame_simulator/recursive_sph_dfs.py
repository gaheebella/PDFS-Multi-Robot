...
# J1 uses exactly the same local parking offset as J0.
# J0: junction center + (-25, -25)
# J1: junction center + (-25, -25)
J1_ANCHOR_PARK_POSITION = (
    J1_CENTER
    + (
        ANCHOR_PARK_POSITION
...
BRANCH_DIRECTIONS = {
    ...
    "J1_UP": pygame.Vector2(0.0, -1.0),
    "J1_DOWN": pygame.Vector2(0.0, 1.0),
}
BRANCH_LENGTHS = {
    "UP": float(normal_length),
    "LEFT": float(normal_length),
    "RIGHT": float(right_length),
    "J1_UP": float(normal_length),
    "J1_DOWN": float(normal_length),
}

NESTED_BRANCHES = {"J1_UP", "J1_DOWN"}
NESTED_REGION_TO_EDGE = {
    "J1_UP": "E_J1_UP",
    "J1_DOWN": "E_J1_DOWN",
}
...
nested_relay_slots: list[dict] = []
nested_relay_edge_id: Optional[str] = None
...
def get_branch_path_regions(
    ...
    """Return physical regions belonging to one active edge path."""

    if branch in NESTED_BRANCHES:
        regions = {
            "RIGHT",
            "J1_JUNCTION",
            branch,
        ...
        if include_bottom:
            regions.update({"JUNCTION", "BOTTOM"})
        return regions
...
def is_region_allowed(position: pygame.Vector2) -> bool:
    ...
    if phase in shared_branch_phases:
        if active_branch in NESTED_BRANCHES:
            return region in {
                "BOTTOM",
                "JUNCTION",
                "RIGHT",
...
CHILD_JUNCTION_GATHER_MARGIN = 34
CHILD_JUNCTION_GATHER_MAX_SPEED = 8.0
CHILD_JUNCTION_GATHER_DWELL = 0.60

NESTED_EDGE_ENTRY_COUNT = 18
NESTED_EDGE_MOUTH_OFFSET = 12.0

NESTED_EDGE_TARGET_COUNT = 18
NESTED_EDGE_REQUIRED_ENTRY_RATIO = 0.90
...
def child_junction_arrival_ready(
    ...
    if (
        arrived_count >= CHILD_JUNCTION_ARRIVAL_COUNT
        and child_junction_handoff_ready(
            robots,
            junction_id,
        )
    ):
...
def get_child_junction_gather_status(
    robots,
    junction_id: str,
) -> dict:
    """Check whether all non-communication robots have gathered."""

    junction_rect = get_junction_proxy_rect(
        junction_id,
    )

    gather_rect = junction_rect.inflate(
        2 * CHILD_JUNCTION_GATHER_MARGIN,
        2 * CHILD_JUNCTION_GATHER_MARGIN,
    )
    ...
    all_gathered = (
        total_count > 0
        and gathered_count == total_count
    )
    ...
    velocity_stable = (
        max_speed
        <= CHILD_JUNCTION_GATHER_MAX_SPEED
    )
...
def branch_progress_position(
    position: pygame.Vector2,
    branch: str,
) -> float:
    return position.dot(BRANCH_DIRECTIONS[branch])

def branch_progress(robot: "Robot", branch: str) -> float:
    return branch_progress_position(robot.position, branch)


def branch_depth_from_junction(
    position: pygame.Vector2,
    branch: str,
) -> float:
    """Distance travelled inside a branch from its active-junction mouth."""

    entrance = get_branch_entrance(branch)
    direction = BRANCH_DIRECTIONS[branch]
    return clamp(
        (position - entrance).dot(direction),
        0.0,
        BRANCH_LENGTHS[branch],
    )

def branch_point_at_depth(
    branch: str,
    depth: float,
) -> pygame.Vector2:
    ...
    return (
        ...
        + BRANCH_DIRECTIONS[branch] * depth
    )
...
def get_saturation_rect(branch: str) -> pygame.Rect:
    ...
    far_depth = max(
        near_depth + 1.0,
        boundary_depth - ROBOT_RADIUS * 2.0,
    )

    entrance = get_branch_entrance(branch)
    direction = BRANCH_DIRECTIONS[branch]

    near_point = entrance + direction * near_depth
    far_point = entrance + direction * far_depth

    if abs(direction.y) > 0.5:
        top = min(near_point.y, far_point.y)
        bottom = max(near_point.y, far_point.y)
        return pygame.Rect(
            int(entrance.x - half_width),
            int(top),
            corridor_width,
            max(1, int(bottom - top)),
        )

    left = min(near_point.x, far_point.x)
    right = max(near_point.x, far_point.x)
    return pygame.Rect(
        int(left),
        int(entrance.y - half_width),
...
def constrain_normal_behind_shepherd_curtain(robot: "Robot") -> None:
    ...
    limit_point = branch_point_at_depth(active_branch, limit_depth)
    direction = BRANCH_DIRECTIONS[active_branch]

    if abs(direction.y) > 0.5:
        robot.position.y = limit_point.y
    else:
        robot.position.x = limit_point.x

    forward_speed = robot.velocity.dot(direction)
    if forward_speed > 0.0:
        robot.velocity -= direction * forward_speed
...
class Robot:
    def __init__(self, x: float, y: float, robot_id: int):
        ...
        self.anchor_entry_times: dict[str, float] = {}
        self.anchor_inside_state: dict[str, bool] = {
            "J0": anchor_election_rect.collidepoint(x, y),
            "J1": J1_ANCHOR_ELECTION_RECT.collidepoint(x, y),
        }
        ...
        self.comm_path_margin = float("-inf")
        self.connected_to_active_anchor = False
        self.active_anchor_hop = -1
        self.active_anchor_path_margin = float("-inf")
        self.local_comm_parent: Optional[object] = None
        self.received_branch = None
        self.received_command = None
        self.received_sequence = -1
...
def j1_local_communication_active() -> bool:
    """Return whether communication must be rooted at the J1 Anchor."""

    return (
        resolve_current_junction_id() == "J1"
        and phase
        in {
            SimulationPhase.STABILIZE_JUNCTION,
            SimulationPhase.EXPLORE_NESTED_EDGE,
...
def is_j1_local_communication_robot(
    robot,
) -> bool:
    """Return whether a robot belongs below the J1 Anchor."""
...
def is_valid_j1_local_parent(
    candidate,
    j1_anchor,
) -> bool:
    """Prevent local robots from attaching to upstream Branch Relays."""

    if candidate is j1_anchor:
        return True

    return is_j1_local_communication_robot(
        candidate,
    )


def rebuild_j1_anchor_communication_tree(
    robots,
) -> None:
    """Root the J1 local communication mesh at the J1 Anchor."""

    if not j1_local_communication_active():
        return

    j1_anchor = get_junction_anchor_robot(
        robots,
        "J1",
    )
    ...
    if (
        j1_anchor is None
        or not j1_anchor.connected_to_base
    ):
        return
    ...
    local_robots = [
        ...
        for robot in robots
        if (
            robot is not j1_anchor
            and is_j1_local_communication_robot(
                robot,
            )
        )
    ]
    ...
    # Base 기준으로 만들어진 기존 부모를 제거한다.
    # J1 Anchor의 Base 연결은 그대로 유지한다.
    for robot in local_robots:
    ...
    while heap:
        ...
        for neighbor in current.comm_neighbors:
            ...
            # J1 로컬 로봇만 탐색한다.
            # BRANCH Relay와 TRUNK Relay는 여기서 제외된다.
            if neighbor_id not in local_robot_ids:
                continue

            if not is_valid_j1_local_parent(
                current,
                j1_anchor,
            ):
                continue
            ...
            edge_margin = (
                COMM_RANGE
                - current.position.distance_to(
                    neighbor.position,
                )
            )

            candidate_margin = min(
                current.comm_path_margin,
                edge_margin,
            )

            if (
                candidate_margin
                <= neighbor.comm_path_margin
                + EPSILON
            ):
                continue
...
def get_active_command_anchor(
    robots,
) -> Optional["Robot"]:
    """Return the Anchor responsible for the current DFS junction."""

    active_junction_id = (
        resolve_current_junction_id()
    )

    if (
        active_junction_id is not None
        and active_junction_id
        != ROOT_JUNCTION_ID
    ):
        child_anchor = (
            get_junction_anchor_robot(
                robots,
                active_junction_id,
            )
        )
...
def is_j1_local_communication_member(
    robot,
) -> bool:
    """Return whether a robot belongs below the J1 Anchor."""
...
def update_active_anchor_communication_tree(
    robots,
) -> None:
    """Build J1 local widest paths rooted at the J1 Anchor.

    Base connectivity is left untouched. Upstream Branch/Trunk relays may feed
    the Anchor, but they are excluded from the J1 local parent tree.
    """

    for robot in robots:
        robot.connected_to_active_anchor = False
        robot.active_anchor_hop = -1
        robot.active_anchor_path_margin = float("-inf")
        robot.local_comm_parent = None

    if resolve_current_junction_id() != "J1":
        return

    active_anchor = get_active_command_anchor(robots)

    if (
        active_anchor is None
        or not active_anchor.connected_to_base
    ):
        return

    active_anchor.connected_to_active_anchor = True
    active_anchor.active_anchor_hop = 0
    active_anchor.active_anchor_path_margin = float("inf")
    active_anchor.local_comm_parent = None

    local_ids = {
        robot.robot_id
        for robot in robots
        if is_j1_local_communication_member(robot)
    }

    heap = [(-1.0e12, active_anchor.robot_id, active_anchor)]
    ...
    while heap:
        ...
        for neighbor in current.comm_neighbors:
            ...
            if not neighbor.connected_to_base:
                continue

            edge_margin = (
                COMM_RANGE
                - current.position.distance_to(neighbor.position)
            )
            candidate_margin = min(
                current.active_anchor_path_margin,
                edge_margin,
            )

            if (
                candidate_margin
                <= neighbor.active_anchor_path_margin + EPSILON
            ):
                continue

            neighbor.connected_to_active_anchor = True
            neighbor.active_anchor_hop = current.active_anchor_hop + 1
            neighbor.active_anchor_path_margin = candidate_margin
            neighbor.local_comm_parent = current
...
def update_communication_system(
    robots,
    grid,
):
    update_communication_neighbors(
        robots,
        grid,
    )

    active_command_anchor = (
        get_active_command_anchor(
            robots,
        )
    )

    propagate_base_message(
        robots,
        active_command_anchor,
    )

    update_active_anchor_communication_tree(
        robots,
    )
...
def get_communication_stats(robots):
    ...
    return {
        ...
        "anchor_connected": bool(
            ...
            and active_anchor.connected_to_base
        ),
    }
...
def draw_communication_links(
    ...
    for robot in robots:
        if (
            j1_local_mode
            and is_j1_local_communication_member(
                robot,
            )
            and robot.connected_to_active_anchor
        ):
            parent = (
                robot.local_comm_parent
            )
        else:
...
def get_relay_path_endpoint(branch):
    ...
    # RIGHT ends at child junction J1. Keep the final Branch Relay exactly
    # one relay spacing before the fixed J1 Anchor park point so the handoff
    # link is present before the swarm enters J1.
    direction = (
        J1_ANCHOR_PARK_POSITION
        - ANCHOR_PARK_POSITION
    )
    ...
    return (
        J1_ANCHOR_PARK_POSITION
        - direction.normalize() * RELAY_SPACING
    )
...
def deploy_relay_for_slot(robots, slot):
    candidate = select_relay_candidate(
        robots,
        slot,
    )
    ...
    candidate.relay_scope = "BRANCH"
    candidate.relay_owner_edge_id = None
...
def child_junction_handoff_ready(
    robots,
    junction_id: str,
) -> bool:
    """Return whether the parent Branch Relay can directly feed the child Anchor."""
    ...
    settled_relays = [
        relay
        for relay in relays
        if relay_at_slot_is_settled(relay)
    ]
    if not settled_relays:
        return False

    nearest_distance = min(
        relay.position.distance_to(
            J1_ANCHOR_PARK_POSITION,
        )
        for relay in settled_relays
    )
...
def update_relay_deployment(
    ...
    if (
        should_deploy
        and relay_deploy_cooldown <= 0.0
        and deploy_relay_for_slot(
            robots,
            next_slot,
        )
    ):
...
def get_junction_anchor_robot(
    ...
    """Return the elected Anchor robot of one topology junction."""
    ...
    if (
        context is None
        or context.anchor_robot_id is None
    ):
        return None
    ...
    return next(
        (
            ...
            for robot in robots
            if robot.robot_id
            == context.anchor_robot_id
        ),
        None,
    )


def get_nested_relay_path_end(edge_id: str) -> pygame.Vector2:
    """Return a side-lane endpoint so the center remains open for the swarm."""

    profile = get_edge_traversal_profile(edge_id)
    rect = profile.corridor_rect

    if abs(profile.direction.y) > 0.5:
        x = rect.left + RELAY_LANE_MARGIN
        y = (
            rect.top + RELAY_END_CLEARANCE
            if profile.direction.y < 0.0
            else rect.bottom - RELAY_END_CLEARANCE
        )
        return pygame.Vector2(x, y)

    y = rect.top + RELAY_LANE_MARGIN
    x = (
        rect.left + RELAY_END_CLEARANCE
        if profile.direction.x < 0.0
        else rect.right - RELAY_END_CLEARANCE
    )
    return pygame.Vector2(x, y)


def get_nested_relay_path_start(edge_id: str) -> pygame.Vector2:
    """Use the source Junction Anchor as the relay-chain root, like J0."""

    profile = get_edge_traversal_profile(edge_id)
    if profile.source_junction_id == "J1":
        return J1_ANCHOR_PARK_POSITION.copy()
    return ANCHOR_PARK_POSITION.copy()
...
def nested_relay_path_progress(
    ...
    """Return progress along the Anchor-rooted nested relay lane."""
    ...
    direction = path_vector / path_length
    return clamp(
        (position - start).dot(direction),
        0.0,
        path_length,
    )


def get_active_nested_relays(
    robots,
    edge_id: Optional[str] = None,
) -> list["Robot"]:
    """Return deployed Nested Relays, optionally limited to one Edge."""
...
def get_next_undeployed_nested_slot(
    robots,
) -> Optional[dict]:
    """Return the first Nested Relay slot that has not been occupied."""

    if nested_relay_edge_id is None:
        return None

    deployed_indices = {
        robot.relay_index
        for robot in get_active_nested_relays(
            robots,
    ...
    return next(
        (
            slot
            for slot in nested_relay_slots
            if slot["index"]
            not in deployed_indices
        ),
        None,
    )


def get_nested_exploration_front_progress(
    robots,
    edge_id: str,
) -> float:
    """Return the leading mobile robot's progress along a nested Edge."""
...
def select_nested_relay_candidate(
    robots,
    slot: dict,
) -> Optional["Robot"]:
    """Select a connected mobile robot near one Nested Relay slot."""
...
def deploy_nested_relay_for_slot(
    robots,
    slot: dict,
) -> bool:
    """Convert one connected mobile robot into a Nested Relay."""

    candidate = select_nested_relay_candidate(
        robots,
        slot,
    )
    ...
    candidate.role = "RELAY"
    candidate.relay_scope = "NESTED"
    candidate.relay_owner_edge_id = (
        slot["edge_id"]
    )
    candidate.relay_anchor = (
        slot["position"].copy()
    )
    candidate.relay_index = slot["index"]
    candidate.velocity.update(0.0, 0.0)
...
def get_nested_front_communication_status(
    robots,
    edge_id: str,
) -> dict:
    """Measure Base connectivity of the leading nested-branch robots."""
...
def update_nested_relay_deployment(
    robots,
    dt: float,
) -> None:
    ...
    nested_relay_deploy_cooldown = max(
        0.0,
    ...
    if (
        ...
        or nested_relay_edge_id is None
    ):
        return
    ...
    source_anchor = get_junction_anchor_robot(
        robots,
        profile.source_junction_id,
    )
    ...
    front_progress = (
        get_nested_exploration_front_progress(
            robots,
            nested_relay_edge_id,
        )
    )
    ...
    status = (
        get_nested_front_communication_status(
            robots,
            nested_relay_edge_id,
        )
    )
    ...
    next_slot = (
        get_next_undeployed_nested_slot(
            robots,
        )
    )
...
def update_active_relay_deployment(robots, dt: float) -> None:
...
def release_nested_relay_into_backtracking(robot, edge_id: str) -> None:
    profile = get_edge_traversal_profile(edge_id)
    old_index = robot.relay_index
    robot.role = "NORMAL"
    robot.relay_anchor = None
    robot.relay_index = -1
    robot.relay_scope = None
    robot.relay_owner_edge_id = None
    robot.velocity = -profile.direction * RELAY_RELEASE_SPEED
    robot.acceleration.update(0.0, 0.0)
    print(
        f"[Nested Relay] retract robot={robot.robot_id}, "
        f"edge={edge_id}, index={old_index}"
    )


def update_nested_relay_retraction(robots, dt: float) -> None:
    global relay_retract_cooldown
    global relay_retract_clear_timer
    ...
    if (
        ...
        or not active_branch_is_nested()
    ):
        relay_retract_clear_timer = 0.0
        return

    edge_id = NESTED_REGION_TO_EDGE[active_branch]
    relays = get_active_nested_relays(robots, edge_id)
    if not relays:
        relay_retract_clear_timer = 0.0
        return

    farthest = relays[-1]
    progress = nested_relay_path_progress(farthest.position, edge_id)
    ...
    mobile = [
        ...
        if robot.role in {"NORMAL", "SHEPHERD"}
        and get_robot_region(robot.position)
        in {active_branch, "J1_JUNCTION", "RIGHT"}
    ]
    ...
    beyond = [
        ...
        and nested_relay_path_progress(robot.position, edge_id)
        > progress + RELAY_PASS_MARGIN
    ]

    clear = (
        not beyond
        and all(robot.connected_to_base for robot in mobile)
        and all(
            robot.connected_to_active_anchor
            for robot in mobile
            if is_j1_local_communication_member(robot)
        )
    )

    relay_retract_clear_timer = (
    ...
    if (
        ...
        release_nested_relay_into_backtracking(farthest, edge_id)
        relay_retract_cooldown = RELAY_RETRACT_COOLDOWN
        relay_retract_clear_timer = 0.0


def update_active_relay_retraction(robots, dt: float) -> None:
    if active_branch_is_nested():
        update_nested_relay_retraction(robots, dt)
    else:
        update_relay_retraction(robots, dt)


def release_nested_relays_for_backtrack(
    robots,
    edge_id: str,
) -> int:
    """Release all relays belonging to one completed nested edge."""
...
def clear_nested_relay_plan() -> None:
    """Clear the completed nested-edge relay plan."""
...
def update_anchor_entry_records(robots, current_time):
    election_rects = {
        "J0": anchor_election_rect,
        "J1": J1_ANCHOR_ELECTION_RECT,
    }

    for robot in robots:
        for junction_id, election_rect in election_rects.items():
            inside = election_rect.collidepoint(
                robot.position.x,
                robot.position.y,
            )
            was_inside = robot.anchor_inside_state.get(junction_id, False)

            if (
                inside
                and not was_inside
                and junction_id not in robot.anchor_entry_times
                and robot.role == "NORMAL"
            ):
                robot.anchor_entry_times[junction_id] = current_time

            robot.anchor_inside_state[junction_id] = inside

        robot.anchor_region_entry_time = robot.anchor_entry_times.get("J0")
        robot.was_in_anchor_region = robot.anchor_inside_state.get("J0", False)
...
def compute_anchor_candidate_scores(
    candidates,
    robots,
    junction_id: str = "J0",
):
    ...
    park_position = (
        ANCHOR_PARK_POSITION
        if junction_id == "J0"
        else J1_ANCHOR_PARK_POSITION
    )
    junction_position = get_physical_junction_center(junction_id)

    entry_times = [
        candidate.anchor_entry_times[junction_id]
        for candidate in candidates
    ]
    min_time, max_time = min(entry_times), max(entry_times)

    max_parking_distance = max(
        candidate.position.distance_to(park_position)
        for candidate in candidates
    )
    ...
    parking_direction = normalized_direction_toward(
        junction_position,
        park_position,
    )

    for candidate in candidates:
        entry_time = candidate.anchor_entry_times[junction_id]
        arrival = (
            1.0
            if max_time - min_time <= EPSILON
            else 1.0 - (entry_time - min_time) / (max_time - min_time)
        )
        parking = 1.0 - candidate.position.distance_to(park_position) / max(
            max_parking_distance,
            EPSILON,
        )
        velocity_direction = (
            candidate.velocity.normalize()
            if candidate.velocity.length_squared() > EPSILON
            else parking_direction
        )
        direction = 0.5 * (
            velocity_direction.dot(parking_direction) + 1.0
        )
...
def elect_anchor_for_junction(
    ...
    first_entry = min(
        robot.anchor_entry_times[junction_id]
        for robot in candidates
    )

    if (
        len(candidates) < ANCHOR_ELECTION_MIN_CANDIDATES
        and simulation_time - first_entry < ANCHOR_ELECTION_WAIT_TIME
    ):
        return None

    compute_anchor_candidate_scores(
        candidates,
        robots,
        junction_id,
    )
    ...
    selected.anchor_position = (
        ANCHOR_PARK_POSITION.copy()
        if junction_id == "J0"
        else J1_ANCHOR_PARK_POSITION.copy()
    )
    ...
    print(
        ...
        f"score={selected.anchor_election_score:.3f}, "
        f"entry={selected.anchor_entry_times[junction_id]:.3f}"
    )
...
def elect_junction_anchor(robots):
    ...
    junction_anchor = elect_anchor_for_junction(
        robots,
        ROOT_JUNCTION_ID,
    )
    return junction_anchor
...
def elect_child_junction_anchor(
    ...
    """Use the same multi-criteria Anchor election as J0."""

    if junction_id not in topology_nodes:
        raise ValueError(f"Unknown junction: {junction_id}")

    return elect_anchor_for_junction(
        robots,
        junction_id,
    )
...
def get_branch_entrance(branch: str) -> pygame.Vector2:
    """Return the active-junction-side mouth of a physical branch."""

    if branch in NESTED_BRANCHES:
        return get_nested_edge_entrance(
            NESTED_REGION_TO_EDGE[branch]
        )
    if branch == "UP":
...
def geodesic_edf_direction(
    ...
    """Analytic free-space route direction for root and nested branches."""
    ...
    if branch in NESTED_BRANCHES:
        ...
        if final_target is None:
            return nested_edge_route_direction(position, profile)

        region = get_robot_region(position)
        if region == branch:
            return normalized_direction_toward(position, final_target)
        if region == "J1_JUNCTION":
            return normalized_direction_toward(
                position,
                get_branch_entrance(branch),
        ...
        if region == "RIGHT":
            return normalized_direction_toward(position, J1_CENTER)
        return pygame.Vector2()
...
def compute_virtual_valve_force(robot: "Robot") -> pygame.Vector2:
    """Close every unselected mouth at the currently active junction."""
    ...
    if active_branch in NESTED_BRANCHES:
        if region != "J1_JUNCTION":
            return force
        junction_center = J1_CENTER
        candidate_branches = NESTED_BRANCHES
    else:
        if region != "JUNCTION":
            return force
        junction_center = pygame.Vector2(center_x, center_y)
...
def complete_nested_branch(
    robots,
    branch: str,
) -> None:
    global active_branch

    edge_id = NESTED_REGION_TO_EDGE[branch]
    edge = topology_edges[edge_id]

    edge.visit_state = VisitState.VISITED
    topology_nodes[edge.end_node_id].visit_state = VisitState.VISITED
    update_current_dfs_frame(edge_id, VisitState.VISITED)

    context = junction_contexts["J1"]
    context.selected_edge_id = None
    context.stable_timer = 0.0
    junction_consensus_tracker.reset()

    child_anchor = get_junction_anchor_robot(robots, "J1")
    if child_anchor is not None:
        child_anchor.selected_branch = None

    clear_nested_relay_plan()
    set_topology_cursor_at_junction("J1")

    # J1에서 다음 자식 Edge를 고르기 전에는 부모 RIGHT 체인을 유지한다.
    active_branch = "RIGHT"

    print(
        f"[Nested Edge Completed] edge={edge_id}, returned_to=J1"
    )


def begin_return_from_child_junction(robots, junction_id: str) -> None:
    global phase
    global active_branch
    global current_node_id
    global current_junction_id
    ...
    frame = get_current_dfs_frame()
    if frame is None or frame.junction_id != junction_id:
        raise RuntimeError(
            f"Cannot return from junction={junction_id}; invalid DFS frame"
        )

    parent_edge_id = frame.parent_edge_id
    if parent_edge_id is None:
        raise RuntimeError(
            f"Junction={junction_id} has no parent edge"
        )

    child_anchor = get_junction_anchor_robot(robots, junction_id)
    if child_anchor is not None:
        child_anchor.role = "NORMAL"
        child_anchor.anchor_position = None
        child_anchor.selected_branch = None
        child_anchor.velocity.update(0.0, 0.0)

    topology_nodes[junction_id].visit_state = VisitState.VISITED
    pop_current_junction_frame()

    parent_edge = topology_edges[parent_edge_id]
    parent_branch = EDGE_TO_BRANCH[parent_edge_id]

    active_branch = parent_branch
    current_node_id = parent_edge.end_node_id
    current_junction_id = parent_edge.start_node_id
    current_edge_id = parent_edge_id

    if junction_anchor is not None:
        junction_anchor.selected_branch = parent_branch

    clear_nested_relay_plan()
    phase = SimulationPhase.FLOW_BACKTRACK

    print(
        f"[DFS] child junction completed={junction_id}; "
        f"backtrack edge={parent_edge_id}"
    )
...
class JunctionConsensusTracker:
    ...
    def update(
        ...
        candidates = [
            ...
            for robot in robots
            if robot.role == "NORMAL"
            and get_robot_region(robot.position) == junction_region
        ]
...
def get_early_capture_rect(branch: str) -> pygame.Rect:
    ...
    if abs(direction.y) > 0.5:
        if direction.y < 0.0:
            return pygame.Rect(
                saturation_rect.left,
                int(get_branch_tip_target(branch).y - 18),
                saturation_rect.width,
                EARLY_CAPTURE_DEPTH,
            )
        return pygame.Rect(
            saturation_rect.left,
            int(get_branch_tip_target(branch).y + 18 - EARLY_CAPTURE_DEPTH),
            saturation_rect.width,
            EARLY_CAPTURE_DEPTH,
        )
    ...
    if direction.x < 0.0:
        return pygame.Rect(
            int(get_branch_tip_target(branch).x - 18),
            saturation_rect.top,
            EARLY_CAPTURE_DEPTH,
            saturation_rect.height,
        )

    return pygame.Rect(
        int(get_branch_tip_target(branch).x + 18 - EARLY_CAPTURE_DEPTH),
        saturation_rect.top,
        EARLY_CAPTURE_DEPTH,
        saturation_rect.height,
    )
...
def build_shepherd_slots(branch, count):
    ...
    direction = BRANCH_DIRECTIONS[branch]
    boundary_center = branch_point_at_depth(
        branch,
        get_shepherd_boundary_depth(branch),
    )
    lateral_direction = pygame.Vector2(-direction.y, direction.x)

    return [
        boundary_center + lateral_direction * value
...
def shepherd_candidates(robots, branch, required_count):
    """Return leading connected robots inside the active capture region."""
...
def capture_region_ready_for_shepherd(robots, branch):
    ...
    if branch in NESTED_BRANCHES:
        edge_id = NESTED_REGION_TO_EDGE[branch]
        front_status = get_nested_front_communication_status(
            robots,
            edge_id,
        )
    else:
...
def compute_route_force(robot):
    region = get_robot_region(robot.position)
    root_junction_target = pygame.Vector2(center_x, center_y)
    active_junction_target = (
        J1_CENTER.copy()
        if resolve_current_junction_id() == "J1"
        else root_junction_target
    )
    force = pygame.Vector2()
    ...
    if phase not in independent and junction_anchor is not None:
        ...
        if phase in branch_phases and robot.received_branch != active_branch:
            return force
    ...
    if phase == SimulationPhase.MOVE_TO_JUNCTION:
        ...
        if robot.role == "SHEPHERD":
            ...
            gain = ROUTE_FORCE if region == active_branch else OUTLET_FORCE
            force = (
        ...
        if region in BRANCHES:
            ...
            if y_distance > 0.0:
                scale = max(
                    RETURN_MIN_FORCE_SCALE,
                    min(1.0, y_distance / RETURN_BRAKE_DISTANCE),
                )
    ...
    elif region in {"J1_UP", "J1_DOWN"}:
        force.x += CENTERING_GAIN * (J1_CENTER.x - robot.position.x)
    elif region in {"LEFT", "RIGHT"}:
...
def update_simulation_state(robots, dt, reference_density, spatial_grid):
    ...
    if phase == SimulationPhase.MOVE_TO_JUNCTION:
        ...
        if int(simulation_time * 2) != int(
            ...
            print(
                f"[Junction Consensus] "
                f"junction={junction_id}, "
                f"n={junction_consensus_tracker.candidate_count}, "
                f"stable={junction_consensus_tracker.stable_ratio:.2f}, "
                f"speed={junction_consensus_tracker.mean_speed:.2f}, "
                f"dRho={junction_consensus_tracker.mean_density_delta_ratio:.3f}, "
        ...
        if (
            ...
            if selected_edge_id is None:
                ...
                begin_return_from_child_junction(
                    robots,
                    junction_id,
                )
            else:
                ...
                active_branch = profile.physical_region
                child_anchor.selected_branch = active_branch
        ...
        update_active_relay_deployment(robots, dt)
        ...
        update_active_relay_retraction(robots, dt)
        ...
        if active_branch_is_nested():
            in_junction = sum(
                get_robot_region(robot.position) == "J1_JUNCTION"
                and robot.role != "ANCHOR"
                for robot in robots
            )
            edge_id = NESTED_REGION_TO_EDGE[active_branch]
            relays_remaining = bool(
                get_active_nested_relays(robots, edge_id)
            )

            if (
                remaining <= BRANCH_CLEAR_LIMIT
                and in_junction >= JUNCTION_SWITCH_COUNT
                and not relays_remaining
            ):
                complete_nested_branch(robots, active_branch)
                phase = SimulationPhase.STABILIZE_JUNCTION
...
def draw_branch_colour_fields(surface):
    """Tint root and nested physical branches using shared identities."""
    ...
    label_positions = {
        ...
        "J1_UP": (J1_CENTER.x, J1_UP_RECT.top + 18),
        "J1_DOWN": (J1_CENTER.x, J1_DOWN_RECT.bottom - 18),
    }
...
def draw_nested_proxy_partition(surface):
    """Draw the latest temporary proxy partition of J1."""

    if (
        last_nested_proxy_junction_id is None
        or not last_nested_proxy_partition
        or not last_nested_proxy_cell_centers
    ):
        return
    ...
    centroids = {
        edge_id: pygame.Vector2()
        for edge_id in last_nested_proxy_candidates
    }
    ...
    for key, edge_id in last_nested_proxy_partition.items():
        center = last_nested_proxy_cell_centers[key]

        color = NESTED_EDGE_COLORS.get(
            edge_id,
            ROBOT_BASE_COLOR,
        )
        ...
        pygame.draw.rect(
            overlay,
            (*color, BRANCH_PROXY_ALPHA),
            rect,
        )

        if edge_id in centroids:
            centroids[edge_id] += center
            counts[edge_id] += 1
    ...
    for key, edge_id in last_nested_proxy_partition.items():
        ...
        left = center.x - PROXY_CELL_SIZE / 2
        right = left + PROXY_CELL_SIZE
        top = center.y - PROXY_CELL_SIZE / 2
        bottom = top + PROXY_CELL_SIZE

        for side, (dc, dr) in neighbour_offsets.items():
            neighbour_edge = (
                last_nested_proxy_partition.get(
                    (col + dc, row + dr)
                )
            )

            if neighbour_edge == edge_id:
                continue
    ...
    for edge_id in last_nested_proxy_candidates:
        ...
        label = small_font.render(
            labels.get(edge_id, edge_id),
            True,
            (255, 255, 255),
        )

        badge = label.get_rect(
            center=(
                round(center.x),
                round(center.y),
            )
...
def draw_nested_proxy_robot_assignments(
    surface,
    robots,
):
    """Draw J1 decision-only robot assignments at projected proxy positions."""

    if (
        last_nested_proxy_junction_id is None
        or not last_nested_proxy_robot_assignment
    ):
        return
    ...
    for robot in robots:
        edge_id = (
            last_nested_proxy_robot_assignment.get(
                robot.robot_id
            )
        )
        ...
        point = project_robot_to_nested_proxy(
            robot.position,
            last_nested_proxy_junction_id,
        )
...
def draw_shepherd_curtain(surface):
    """Visualize the full-width gate for root and nested branches."""
    ...
    center = branch_point_at_depth(active_branch, depth)
    direction = BRANCH_DIRECTIONS[active_branch]
    lateral = pygame.Vector2(-direction.y, direction.x)
    half = half_width - 2

    start = center - lateral * half
    end = center + lateral * half
...
while running:
    ...
    if show_regions:
        ...
        draw_proxy_robot_assignments(
            screen,
            robots,
        )

        # J1 Proxy
        draw_nested_proxy_partition(
            screen,
        )
        draw_nested_proxy_robot_assignments(
            screen,
            robots,
        )
        ...
        if active_branch in {"UP", "LEFT", "J1_UP", "J1_DOWN"}:
            pygame.draw.rect(
                screen,
                BRANCH_COLORS[active_branch],
                get_early_capture_rect(active_branch),
                width=2,
            )
            pygame.draw.rect(
                screen,
