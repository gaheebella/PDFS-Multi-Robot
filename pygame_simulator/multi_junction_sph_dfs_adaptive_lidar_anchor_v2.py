...
# ---------------------------------------------------------
# Localization-free temporary lateral leakage suppression
# ---------------------------------------------------------

TEMP_GUARD_EXPANSION_WINDOW = 12

# 횡방향 spread가 baseline보다 얼마나 증가해야
# opening 후보로 볼지.
TEMP_GUARD_SPREAD_RATIO = 1.10

# 한 프레임만 퍼지는 noise를 막기 위한 persistence.
TEMP_GUARD_MIN_STREAK = 4

# 임시 벽에 사용할 최소/최대 로봇 수.
TEMP_GUARD_MIN_ROBOTS = 6
TEMP_GUARD_MAX_ROBOTS = 18

# TEMP_GUARD 이웃 간 목표 거리.
TEMP_GUARD_SPACING = 4.5

# 상대위치 제어 gain.
TEMP_GUARD_K_SPRING = 8.0
TEMP_GUARD_K_DAMPING = 2.0
...
class MultiJunctionManager:
    ...
    def __init__(self) -> None:
        ...
        self.temp_guard_active = False
        self.temp_guard_branch_uid: str | None = None
        self.temp_guard_robot_ids: list[int] = []

        self.temp_guard_positive_ids: list[int] = []
        self.temp_guard_negative_ids: list[int] = []    

        self.temp_guard_reference_id: int | None = None
        self.temp_guard_positive_reference_offset: pygame.Vector2 | None = None
        self.temp_guard_negative_reference_offset: pygame.Vector2 | None = None

        self.temp_guard_lateral_axis: pygame.Vector2 | None = None
        self.temp_guard_branch_axis: pygame.Vector2 | None = None

        self.temp_guard_spread_history: list[float] = []
        self.temp_guard_expansion_streak = 0
        self.temp_guard_baseline_spread: float | None = None
...
# =========================================================
# Localization-free TEMP_GUARD system
# =========================================================

def detect_relative_lateral_expansion(
    physical: types.ModuleType,
    robots: Sequence[Any],
) -> tuple[
    bool,
    ...
    """Detect local lateral swarm expansion.

    Decision information:
    - inter-robot relative position,
    - inter-robot neighborhood,
    - frozen local branch direction.

    Forbidden:
    - Junction global coordinate,
    - branch rectangle,
    - walkable mask as Junction evidence,
    ...
    lateral_motion_scores: list[
        tuple[float, Any]
    ] = []

    for robot in normal_robots:
        ...
        # -------------------------------------------------
        # Simulator implementation of relative measurement.
        #
        # Algorithmically this is r_ij:
        # relative position of peer j seen by robot i.
        # Absolute robot x/y is not used as a decision value.
        # -------------------------------------------------
        relative_laterals = [
            float(
                (
        ...
        lateral_span = (
            max(relative_laterals)
            - min(relative_laterals)
        )

        lateral_motion_scores.append(
            (
                lateral_span,
                robot,
            )
    ...
    current_spread = float(
        np.median(
            [
                span
                for span, _
                in lateral_motion_scores
            ]
        )
    )

    history = (
        multi_dfs.temp_guard_spread_history
    )

    # -------------------------------------------------
    # Phase 1:
    # Learn the NORMAL straight-corridor spread once.
    #
    # IMPORTANT:
    # After this baseline is established, do NOT keep
    # adapting it while the swarm enters a Junction.
    # Otherwise gradual lateral expansion is absorbed
    # into the baseline itself.
    # -------------------------------------------------
    if (
        multi_dfs.temp_guard_baseline_spread
        is None
    ):
        ...
        print(
            "[TempGuardBaselineFrozen] "
            f"branch={active_uid} "
            f"baseline={baseline:.3f} "
            f"samples={len(history)} "
            "source=RELATIVE_STRAIGHT_CORRIDOR_SPREAD"
        )
    ...
    expansion_ratio = (
        ...
        # -------------------------------------------------
    # Reject one-sided or stationary deformation.
    #
    # A genuine provisional Junction signal should show
    # robots expanding toward BOTH lateral sides while
    # the local swarm is still moving forward.
    # -------------------------------------------------

    reference = normal_robots[0]

    relative_samples: list[
        tuple[float, float]
    ] = []
    ...
    for (
        ...
        # +normal side moving farther toward +normal.
        if (
            centered_lateral > 0.0
            and lateral_velocity > 0.5
        ):
            positive_outward += 1
    ...
    expansion_evidence = (
        expansion_ratio
        >= TEMP_GUARD_SPREAD_RATIO
        and bilateral_expansion
        and forward_motion
    )
    ...
    detected = (
        multi_dfs.temp_guard_expansion_streak
        >= TEMP_GUARD_MIN_STREAK
    )

    # Robots whose local neighborhood shows the greatest
    # lateral expansion become TEMP_GUARD candidates.
    cohort = [
        robot
        for _, robot in sorted(
            lateral_motion_scores,
            key=lambda item: (
                -item[0],
                item[1].robot_id,
            ),
    ...
    if (
        ...
        print(
            "[RelativeLateralExpansion] "
            f"branch={active_uid} "
            f"spread={current_spread:.3f} "
            f"baseline={baseline:.3f} "
            f"ratio={expansion_ratio:.3f} "
            f"positive_outward={positive_outward} "
            f"negative_outward={negative_outward} "
            f"forward_speed={median_forward_speed:.2f} "
            f"bilateral={bilateral_expansion} "
            f"streak="
            f"{multi_dfs.temp_guard_expansion_streak}/"
            f"{TEMP_GUARD_MIN_STREAK} "
            f"detected={detected}"
        )
...
def activate_temp_guard(
    physical: types.ModuleType,
    tangent: pygame.Vector2,
    normal: pygame.Vector2,
    cohort: Sequence[Any],
) -> None:
    """Create two localization-free TEMP_GUARD walls.

    +normal leakage -> positive TEMP wall
    -normal leakage -> negative TEMP wall

    Each wall is ordered along the parent-branch tangent,
    so for J0-B2 -> J1 the two walls become approximately
    horizontal barriers across the upper/lower openings.
    """

    if multi_dfs.temp_guard_active:
        return
    ...
    candidates = [
        ...
        if (
            ...
            not in {
                multi_dfs.child_probe_lidar_id,
                getattr(physical, "initial_lidar_robot_id", None),
            }
            and not getattr(
                robot,
                "is_fixed_anchor",
    ...
    # -------------------------------------------------
    # Relative lateral coordinate.
    #
    # All values are relative to one robot.
    # Absolute world x/y is not used as the decision.
    # -------------------------------------------------
    lateral_samples = [
        (float((robot.position - candidates[0].position).dot(normal)), robot)
        for robot in candidates
    ]
    median_lateral = float(np.median([value for value, _ in lateral_samples]))
    reference = min(lateral_samples, key=lambda item: abs(item[0] - median_lateral))[1]
    multi_dfs.temp_guard_reference_id = reference.robot_id

    relative_lateral: list[
        tuple[float, Any]
    ] = []
    ...
    positive_candidates = [
        ...
        for value, robot
        in relative_lateral
        if value > lateral_center
    ]
    positive_candidates.sort(
        key=lambda robot: float((robot.position - reference.position).dot(normal)),
        reverse=True,
    )
    ...
    # 18 total이면 기본적으로 9 + 9.
    max_per_side = max(
        1,
        TEMP_GUARD_MAX_ROBOTS // 2,
    )
    ...
    min_per_side = max(
        2,
        TEMP_GUARD_MIN_ROBOTS // 2,
    )

    if (
        len(positive_candidates)
        < min_per_side
        or len(negative_candidates)
        < min_per_side
    ):
        return
    ...
    # IMPORTANT:
    #
    # Each leakage wall must span ACROSS its outgoing
    # opening, therefore order each side along tangent,
    # not along normal.
    # -------------------------------------------------
    ...
    def tangent_order(
        ...
        return sorted(
            ...
            key=lambda robot: float(
                (
                    robot.position
                    - group_reference.position
                ).dot(tangent)
            ),
        )

    positive_selected = tangent_order(
        positive_candidates
    )

    negative_selected = tangent_order(
        negative_candidates
    )

    # -------------------------------------------------
    # Assign chain topology independently for each wall.
    # -------------------------------------------------

    def assign_temp_chain(
        group: Sequence[Any],
        side: str,
    ) -> None:
        ...
        for index, robot in enumerate(
            ...
            robot.role = "TEMP_GUARD"

            robot.temp_guard_group = side
            robot.temp_guard_index = index

            robot.temp_guard_left_id = (
                group[index - 1].robot_id
                if index > 0
                else None
            )

            robot.temp_guard_right_id = (
                group[index + 1].robot_id
    ...
    def group_offset(group: Sequence[Any]) -> pygame.Vector2:
        if not group:
            return pygame.Vector2()
        return sum(
            (robot.position - reference.position for robot in group),
            pygame.Vector2(),
        ) / len(group)

    multi_dfs.temp_guard_positive_reference_offset = group_offset(positive_selected)
    multi_dfs.temp_guard_negative_reference_offset = group_offset(negative_selected)

    multi_dfs.temp_guard_positive_ids = [
        robot.robot_id
        for robot in positive_selected
    ]

    multi_dfs.temp_guard_negative_ids = [
        robot.robot_id
        for robot in negative_selected
    ]

    multi_dfs.temp_guard_robot_ids = (
        multi_dfs.temp_guard_positive_ids
        + multi_dfs.temp_guard_negative_ids
    )

    multi_dfs.temp_guard_active = True

    multi_dfs.temp_guard_branch_uid = (
        getattr(
            physical,
            "active_branch_uid",
            None,
    ...
    multi_dfs.temp_guard_branch_axis = (
        tangent.copy()
    )

    multi_dfs.temp_guard_lateral_axis = (
        normal.copy()
    )
    ...
    print(
        "[TempGuardActivated] "
        f"branch="
        f"{multi_dfs.temp_guard_branch_uid} "
        f"positive="
        f"{multi_dfs.temp_guard_positive_ids} "
        f"negative="
        f"{multi_dfs.temp_guard_negative_ids} "
        f"total="
        f"{len(multi_dfs.temp_guard_robot_ids)} "
        "walls=2 "
        "chain_axis=PARENT_TANGENT "
        "ordering=RELATIVE "
        "world_anchor=False "
        "localization=False"
    )

def update_temp_guard_relative_control(
    physical: types.ModuleType,
    robots: Sequence[Any],
) -> None:
    """Maintain two TEMP_GUARD walls by relative geometry only."""

    if not multi_dfs.temp_guard_active:
        return
    ...
    reference_robot = by_id.get(multi_dfs.temp_guard_reference_id)
    ...
    for robot_id in list(
        ...
        # -------------------------------------------------
        # Keep each TEMP wall stretched across the opening.
        #
        # Chain axis = parent tangent.
        # -------------------------------------------------
        for peer_id, sign in (
            (
                left_id,
            ...
            force += (
                ...
                * (
                    TEMP_GUARD_K_SPRING
                    * spacing_error
                    + TEMP_GUARD_K_DAMPING
                    * tangent_relative_speed
                )
            )

        # -------------------------------------------------
        # Suppress continued outward leakage.
        #
        # POSITIVE wall:
        #     +normal velocity is outward.
        #
        # NEGATIVE wall:
        #     -normal velocity is outward.
        #
        # This does not drive robots to a world coordinate;
        # it only damps their outward local velocity.
        # -------------------------------------------------
        ...
        reference_velocity = (
            reference_robot.velocity
            if reference_robot is not None
            else pygame.Vector2()
        )
        normal_speed = float((robot.velocity - reference_velocity).dot(normal))

        outward_speed = 0.0

        if (
            group == "POSITIVE"
            and normal_speed > 0.0
        ):
            outward_speed = (
                normal_speed
            )
        ...
        if reference_robot is not None:
            group_ids = (
                multi_dfs.temp_guard_positive_ids
                if group == "POSITIVE"
                else multi_dfs.temp_guard_negative_ids
            )
            members = [by_id[item] for item in group_ids if item in by_id]
            if members:
                current_offset = sum(
                    (member.position - reference_robot.position for member in members),
                    pygame.Vector2(),
                ) / len(members)
                baseline = (
                    multi_dfs.temp_guard_positive_reference_offset
                    if group == "POSITIVE"
                    else multi_dfs.temp_guard_negative_reference_offset
                )
                if baseline is not None:
                    drift = current_offset - baseline
                    rel_velocity = robot.velocity - reference_robot.velocity
                    force += (
                        -tangent * (TEMP_GUARD_K_SPRING * 0.35 * drift.dot(tangent))
                        -normal * (TEMP_GUARD_K_SPRING * 0.35 * 1.8 * drift.dot(normal))
                        -rel_velocity * (TEMP_GUARD_K_DAMPING * 0.75)
                    )

        limit_base = getattr(physical, "MAX_ACCELERATION", None)
        if limit_base is None:
            limit_base = getattr(physical, "MAX_ACCEL", 100.0)
        force_limit = 0.18 * float(limit_base)
        if force.length() > force_limit > 0.0:
            force.scale_to_length(force_limit)

        robot.acceleration += force

def release_temp_guard_after_confirmation(
    robots: Sequence[Any],
) -> None:
    """Release provisional blockers after LiDAR confirms the Child Junction."""
    ...
    for robot in robots:
        ...
        if robot.role != "TEMP_GUARD":
            continue

        robot.role = "NORMAL"

        robot.temp_guard_index = None
        robot.temp_guard_group = None
        robot.temp_guard_left_id = None
        robot.temp_guard_right_id = None
    ...
    print(
        "[TempGuardReleased] "
        f"robots={sorted(released_ids)} "
        "reason=CHILD_JUNCTION_CONFIRMED "
        "next=FORM_OFFICIAL_GUARDS"
    )

    multi_dfs.temp_guard_active = False
    multi_dfs.temp_guard_branch_uid = None
    multi_dfs.temp_guard_robot_ids = []

    multi_dfs.temp_guard_positive_ids = []
    multi_dfs.temp_guard_negative_ids = []
    multi_dfs.temp_guard_reference_id = None
    multi_dfs.temp_guard_positive_reference_offset = None
    multi_dfs.temp_guard_negative_reference_offset = None

    multi_dfs.temp_guard_branch_axis = None
    multi_dfs.temp_guard_lateral_axis = None

    multi_dfs.temp_guard_spread_history.clear()

    multi_dfs.temp_guard_baseline_spread = None

    multi_dfs.temp_guard_expansion_streak = 0


def update_temp_guard_system(
    physical: types.ModuleType,
    robots: Sequence[Any],
) -> None:
    """Detect and maintain TEMP_GUARD before Child confirmation."""

    # TEMP guard is meaningful only while exploring
    # one real branch.
    if (
        physical.phase
    ...
    # -------------------------------------------------
    # Active DFS branch changed.
    #
    # A TEMP_GUARD belongs only to the branch on which
    # its lateral-expansion evidence was observed.
    #
    # If DFS leaves that branch before a Child Junction
    # is confirmed, the provisional interpretation was
    # not accepted. Release the old TEMP_GUARD and learn
    # a fresh straight-corridor baseline for the new
    # branch.
    # -------------------------------------------------
    if (
        multi_dfs.temp_guard_branch_uid
        is not None
        and multi_dfs.temp_guard_branch_uid
        != active_uid
    ):
        ...
        if multi_dfs.temp_guard_active:
            ...
            print(
                ...
                f"reason=ACTIVE_BRANCH_CHANGED "
    ...
    # -------------------------------------------------
    # Start TEMP_GUARD observation for the current
    # active branch.
    # -------------------------------------------------
    if (
        multi_dfs.temp_guard_branch_uid
        ...
        print(
            "[TempGuardBranchReset] "
            f"branch={active_uid} "
            "baseline_reset=True"
        )

    # Once Child Junction is confirmed, the official
    # Child Guard pipeline owns the Junction.
    if multi_dfs.child_confirmed:
        return
    ...
    if not multi_dfs.temp_guard_active:
        ...
        (
            ...
            normal,
            cohort,
        ) = detect_relative_lateral_expansion(
            physical,
            robots,
        )
    ...
    update_temp_guard_relative_control(
        physical,
        robots,
    )
...
def update_child_stationary_verification(
    ...
    # -------------------------------------------------
    # Junction has now been confirmed by stationary LiDAR.
    #
    # TEMP_GUARD was only a localization-free leakage
    # suppression mechanism. Release it now so the
    # official Child Guard pipeline can recruit robots.
    # -------------------------------------------------
    release_temp_guard_after_confirmation(
        perception.robots
    )
...
class DarkRenderer:
    ...
    def _draw_map(self, robots: Sequence[Any], perception: AdaptivePerception, show_rays: bool, show_comm: bool, density: bool) -> None:
        ...
        role_colors = {
            ...
            "TEMP_GUARD": (0, 255, 255),
            "JUNCTION_GUARD": COLORS["guard"],
            "FRONTIER_SHEPHERD": COLORS["frontier"],
            "SHEPHERD": COLORS["shepherd"],
            "PRE_SHEPHERD": COLORS["shepherd"],
            "PEBBLE": COLORS["pebble"],
            "RELAY": COLORS["relay"],
            "TRUNK_RELAY": COLORS["trunk"],
        }
        order = {"NORMAL": 0, "TEMP_GUARD": 4, "RELAY": 1, "TRUNK_RELAY": 1, "PEBBLE": 2, "JUNCTION_GUARD": 3, "FRONTIER_SHEPHERD": 4, "PRE_SHEPHERD": 5, "SHEPHERD": 5}
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    while running:
        ...
        if not paused:
            ...
            # ---------------------------------------------
            # Localization-free provisional Junction wall.
            #
            # SPH force 계산 후,
            # robot.update(dt) 전에 relative correction을
            # 추가해야 한다.
            # ---------------------------------------------
            update_temp_guard_system(
                physical,
                robots,
            )
