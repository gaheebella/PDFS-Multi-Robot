...
class AnchorShadowManager:
    ...
    def _elect(self, observed, timestamp: float) -> None:
        ...
        self._lifecycle_row(
            timestamp,
            "ACTIVE",
            activation_reason="valid_pre_election_temporal_reference_and_fixed_pose",
        )
        print(
            f"[AnchorShadow] ACTIVE robot={anchor.robot_id}, "
    ...
    def update(self, robots, timestamp: float, dt: float) -> None:
        ...
        if not self.lifecycle_rows:
            self._lifecycle_row(timestamp, "SEARCHING")
