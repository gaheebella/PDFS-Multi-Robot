"""SPH-based single-junction provisional-Anchor confirmation prototype.
...
7. A lateral-expansion candidate is fixed only as a provisional Anchor. An
   Anchor-local 360-degree LiDAR scan must show graph degree >= 3 before the
   existing Relative Swarm Branch validator starts in shadow mode.
...
multi-junction DFS-tree repair. Point Cloud confirmation remains read-only with
respect to the SPH/DFS controller.
"""
...
from junction_detection.integration.anchor_pointcloud_junction_confirmation import (
    AnchorJunctionState,
    AnchorLocalScan,
    AnchorPointCloudConfirmation,
    save_confirmation_artifacts,
    simulate_polygon_lidar,
)
from junction_detection.integration.anchor_opening_swarm_diagnostics import (
    CohortInput,
    OpeningInterval,
    OpeningSwarmDiagnostics,
    RobotTrendInput,
)
...
JUNCTION_ANCHOR_COLOR = (0, 210, 180)
ANCHOR_SHADOW_DIRECTION_COLOR = (255, 45, 190)
ANCHOR_SHADOW_REFERENCE_COLOR = (10, 235, 235)
POINTCLOUD_OPENING_COLOR = (255, 145, 20)
...
class Robot:
    def __init__(self, x: float, y: float, robot_id: int):
        ...
        self.role = "NORMAL"
        # Shadow-only stationary pose, populated only for an elected Anchor.
        self.anchor_shadow_fixed_position: Optional[pygame.Vector2] = None
    ...
    def update(self, dt: float):
        ...
        if (
            self.role == "JUNCTION_ANCHOR"
            and self.anchor_shadow_fixed_position is not None
        ):
            # Keep the Anchor in communication and density interactions while
            # excluding it from force integration and all ordinary role logic.
            self.position = self.anchor_shadow_fixed_position.copy()
    ...
    def draw(self, surface, color_reference_density, show_density_color):
        ...
        if self.role == "PEBBLE":
            color = PEBBLE_COLOR
        elif self.role == "JUNCTION_ANCHOR":
            color = JUNCTION_ANCHOR_COLOR
...
# =========================================================
# 8-1. Anchor-relative Branch validation (shadow only)
# =========================================================


def _anchor_shadow_env_flag(name: str, default: bool = False) -> bool:
    """Read a strict 0/1 environment flag without silently accepting typos."""
...
ANCHOR_SHADOW_ENABLED = _anchor_shadow_env_flag("SPH_DFS_ANCHOR_SHADOW")
ANCHOR_SHADOW_VERBOSE = _anchor_shadow_env_flag("SPH_DFS_ANCHOR_VERBOSE")
ANCHOR_SHADOW_SAMPLE_INTERVAL_S = float(
    os.environ.get("SPH_DFS_ANCHOR_SAMPLE_INTERVAL_S", "0.1")
)
...
OPENING_SWARM_DIAGNOSTICS_ENABLED = _anchor_shadow_env_flag(
    "SPH_DFS_OPENING_SWARM_DIAGNOSTICS"
)
OPENING_SWARM_DIAGNOSTICS_OUTPUT_DIR = Path(
    os.environ.get(
        "SPH_DFS_OPENING_SWARM_DIAGNOSTICS_OUTPUT_DIR",
        "/tmp/pdfs_opening_swarm_diagnostics",
    )
)
if OPENING_SWARM_DIAGNOSTICS_ENABLED and not (
    ANCHOR_SHADOW_ENABLED and PROVISIONAL_CONFIRMATION_ENABLED
):
    raise ValueError(
        "SPH_DFS_OPENING_SWARM_DIAGNOSTICS=1 requires Anchor shadow and "
        "provisional confirmation"
    )

# The pre-election path must span enough simulated time to reject a single
# velocity sample. Distances remain Pygame world units because this simulator
# defines no authoritative pixel/world-unit to metre conversion.
ANCHOR_SHADOW_HISTORY_WINDOW_S = 3.0
ANCHOR_SHADOW_REFERENCE_MIN_DURATION_S = 0.75
ANCHOR_SHADOW_REFERENCE_MIN_DISPLACEMENT = ROBOT_RADIUS * 2.0
ANCHOR_SHADOW_REFERENCE_MIN_STRAIGHTNESS = 0.60
ANCHOR_SHADOW_HISTORY_MIN_STEP = ROBOT_RADIUS * 0.01
ANCHOR_SHADOW_RANGE_INPUT_UNIT = "pygame_world_unit"
ANCHOR_SHADOW_REFERENCE_LINE_LENGTH = 55.0 * MAP_SCALE
ANCHOR_SHADOW_COHORT_LINE_LENGTH = 75.0 * MAP_SCALE


@dataclass
class AnchorShadowManager:
    """Own the SEARCHING→ELECTED→ACTIVE→RELEASED shadow lifecycle.

    Inference-visible records contain no phase, role, map, Branch label, global
    coordinate, or expected count. Global positions are consumed only here to
    emulate relative sensing. Existing Branch semantics are read only when the
    run summary and evaluation-only plot are written.
    """

    state: str = field(default_factory=lambda: (
        AnchorJunctionState.NO_ANCHOR.value
        if PROVISIONAL_CONFIRMATION_ENABLED else "SEARCHING"
    ))
    anchor_robot_id: Optional[int] = None
    fixed_position: Optional[pygame.Vector2] = None
    reference_yaw_rad: Optional[float] = None
    reference_history_duration_s: float = 0.0
    reference_displacement: float = 0.0
    activation_timestamp: Optional[float] = None
    release_timestamp: Optional[float] = None
    last_sample_timestamp: float = float("-inf")
    evidence_forward: Optional[pygame.Vector2] = None
    evidence_baseline_lateral_variance: float = 0.0
    evidence_lateral_variance: float = 0.0
    evidence_expansion_ratio: float = 1.0
    evidence_expansion_dwell: float = 0.0
    evidence_observation_count: int = 0
    history_by_robot: dict[int, deque] = field(default_factory=dict)
    validator: Optional[RelativeSwarmBranchValidator] = None
    latest_result: Optional[object] = None
    latest_observations: tuple[RobotObservation, ...] = ()
    lifecycle_rows: list[dict] = field(default_factory=list)
    observation_rows: list[dict] = field(default_factory=list)
    trend_rows: list[dict] = field(default_factory=list)
    branch_rows: list[dict] = field(default_factory=list)
    anchor_diagnostic_rows: list[dict] = field(default_factory=list)
    neighbor_diagnostic_rows: list[dict] = field(default_factory=list)
    branch_count_history: list[tuple[float, int]] = field(default_factory=list)
    motion_count_history: list[dict] = field(default_factory=list)
    maximum_position_drift: float = 0.0
    anchor_connected_before_election: bool = False
    anchor_neighbors_before_election: int = 0
    maximum_nearby_contact_count: int = 0
    observation_frame_count: int = 0
    maximum_detected_branch_count: int = 0
    branch_count_change_count: int = 0
    previous_branch_count: Optional[int] = None
    outputs_saved: bool = False
    invalid_reference_recorded: bool = False
    pointcloud_confirmation: AnchorPointCloudConfirmation = field(
        default_factory=AnchorPointCloudConfirmation
    )
    pointcloud_scan: Optional[AnchorLocalScan] = None
    opening_swarm_diagnostics: OpeningSwarmDiagnostics = field(
        default_factory=OpeningSwarmDiagnostics
    )

    def reset(self) -> None:
        """Reset one single-junction shadow run without retaining history."""
        fresh = AnchorShadowManager()
        self.__dict__.clear()
    ...
    def _lifecycle_row(
        ...
        *,
        ...
        self.lifecycle_rows.append({
            "timestamp": timestamp,
            "state": state,
            "anchor_robot_id": "" if self.anchor_robot_id is None else self.anchor_robot_id,
            "election_reason": election_reason,
            "fixed_x": "" if fixed is None else fixed.x,
            "fixed_y": "" if fixed is None else fixed.y,
            "reference_yaw_deg": (
                "" if self.reference_yaw_rad is None
                else math.degrees(self.reference_yaw_rad)
            ),
            "reference_direction_source": (
                "pre_election_magnitude_weighted_temporal_displacements"
                if self.reference_yaw_rad is not None else ""
            ),
            "reference_history_duration_s": self.reference_history_duration_s,
            "reference_displacement": self.reference_displacement,
            "activation_reason": activation_reason,
            "release_reason": release_reason,
            "range_input_unit": ANCHOR_SHADOW_RANGE_INPUT_UNIT,
        })

    def _update_position_histories(self, robots, timestamp: float) -> None:
        """Store only recent poses needed to emulate local temporal direction."""
        live_ids = set()
        for robot in robots:
            if robot.role != "NORMAL" or not robot.connected_to_base:
                continue
            live_ids.add(robot.robot_id)
            history = self.history_by_robot.setdefault(robot.robot_id, deque())
            history.append((timestamp, robot.position.copy()))
            cutoff = timestamp - ANCHOR_SHADOW_HISTORY_WINDOW_S
            while len(history) > 1 and history[0][0] < cutoff:
                history.popleft()
        for robot_id in tuple(self.history_by_robot):
            if robot_id not in live_ids:
                del self.history_by_robot[robot_id]

    def _temporal_direction(
        self, robot_id: int
    ) -> Optional[tuple[float, float, float, float]]:
        """Return robust yaw, duration, path length, and straightness."""
        history = tuple(self.history_by_robot.get(robot_id, ()))
        if len(history) < 3:
            return None
        duration = history[-1][0] - history[0][0]
        if duration < ANCHOR_SHADOW_REFERENCE_MIN_DURATION_S:
            return None
        vector_sum = pygame.Vector2()
        path_length = 0.0
        for (_, first), (_, second) in zip(history, history[1:]):
            displacement = second - first
            magnitude = displacement.length()
            if magnitude < ANCHOR_SHADOW_HISTORY_MIN_STEP:
                continue
            # This is a magnitude-weighted circular mean: normalized headings
            # are accumulated with their travelled distance as weight.
            vector_sum += displacement.normalize() * magnitude
            path_length += magnitude
        if path_length < ANCHOR_SHADOW_REFERENCE_MIN_DISPLACEMENT:
            return None
        resultant = vector_sum.length()
        straightness = resultant / max(path_length, EPSILON)
        if (
            resultant < ANCHOR_SHADOW_REFERENCE_MIN_DISPLACEMENT
            or straightness < ANCHOR_SHADOW_REFERENCE_MIN_STRAIGHTNESS
        ):
            return None
        return (
            math.atan2(vector_sum.y, vector_sum.x),
            duration,
            path_length,
            straightness,
        )

    def _local_expansion_evidence(self, robots, dt: float):
        """Reproduce the existing pre-Branch expansion evidence locally.

        Unlike ``SequentialJunctionInferenceTracker.update``, this shadow copy
        deliberately does not call ``get_robot_region``. It reuses the existing
        observation radius, front quantile, minimum sample count, variance delta,
        expansion ratio, and dwell constants before the original tracker's
        Branch-labelled cohort loop begins.
        """
        mobile = [
        ...
        if len(mobile) < JUNCTION_MIN_OBSERVATION_ROBOTS:
            self.evidence_expansion_dwell = 0.0
            return ()

        moving = [
            robot.observed_velocity for robot in mobile
            if robot.observed_velocity.length() >= JUNCTION_COHORT_MIN_SPEED
        ]
        measured = sum(moving, pygame.Vector2()) if moving else pygame.Vector2()
        if measured.length_squared() > EPSILON:
            measured = measured.normalize()
            if self.evidence_forward is None:
                self.evidence_forward = measured
            elif measured.dot(self.evidence_forward) > 0.0:
                blended = self.evidence_forward * 0.94 + measured * 0.06
                if blended.length_squared() > EPSILON:
                    self.evidence_forward = blended.normalize()
        if self.evidence_forward is None:
            return ()

        swarm_center = sum(
            (robot.position for robot in mobile), pygame.Vector2()
        ) / len(mobile)
        projections = [
            (robot.position - swarm_center).dot(self.evidence_forward)
            for robot in mobile
        ]
        threshold = linear_quantile(projections, JUNCTION_FRONT_QUANTILE)
        front = [
            robot for robot, projection in zip(mobile, projections)
            if projection >= threshold
        ]
        if not front:
            return ()
        front_center = sum(
            (robot.position for robot in front), pygame.Vector2()
        ) / len(front)
        observed = tuple(
        ...
        self.evidence_observation_count = len(observed)
        if len(observed) < JUNCTION_MIN_OBSERVATION_ROBOTS:
            self.evidence_expansion_dwell = 0.0
            return ()

        observation_center = sum(
            (robot.position for robot in observed), pygame.Vector2()
        ) / len(observed)
        lateral = pygame.Vector2(-self.evidence_forward.y, self.evidence_forward.x)
        coordinates = [
            (robot.position - observation_center).dot(lateral)
            for robot in observed
        ]
        self.evidence_lateral_variance = sum(
            value * value for value in coordinates
        ) / len(coordinates)
        if self.evidence_baseline_lateral_variance <= EPSILON:
            self.evidence_baseline_lateral_variance = max(
                self.evidence_lateral_variance,
                JUNCTION_LATERAL_EXPANSION_MIN,
            )
        self.evidence_expansion_ratio = (
            self.evidence_lateral_variance
            / max(self.evidence_baseline_lateral_variance, EPSILON)
        )
        expanding = (
            self.evidence_lateral_variance
            - self.evidence_baseline_lateral_variance
            >= JUNCTION_LATERAL_EXPANSION_MIN
            and self.evidence_expansion_ratio >= JUNCTION_LATERAL_EXPANSION_RATIO
        )
        self.evidence_expansion_dwell = (
            self.evidence_expansion_dwell + dt if expanding else 0.0
        )
        if (
            not expanding
            and self.evidence_lateral_variance
            < self.evidence_baseline_lateral_variance
        ):
            self.evidence_baseline_lateral_variance += JUNCTION_BASELINE_ALPHA * (
                self.evidence_lateral_variance
                - self.evidence_baseline_lateral_variance
            )
        return observed

    def _elect(self, observed, timestamp: float) -> None:
        """Elect one deterministic real robot from local evidence participants."""
        observation_center = sum(
            (robot.position for robot in observed), pygame.Vector2()
        ) / len(observed)
        candidates = []
        for robot in observed:
            direction = self._temporal_direction(robot.robot_id)
            if (
                direction is None
                or robot.role != "NORMAL"
                or robot.is_branch_leader
                or not robot.connected_to_base
            ):
                continue
            history = tuple(self.history_by_robot[robot.robot_id])
            speeds = [
                (second_position - first_position).length()
                / max(second_time - first_time, EPSILON)
                for (first_time, first_position), (second_time, second_position)
                in zip(history, history[1:])
                if second_time > first_time
            ]
            mean_speed = sum(speeds) / max(len(speeds), 1)
            speed_std = math.sqrt(
                sum((speed - mean_speed) ** 2 for speed in speeds)
                / max(len(speeds), 1)
            )
            robot_neighbor_count = sum(
                getattr(peer, "role", None) != "BASE"
                for peer in robot.comm_neighbors
            )
            # Lexicographic score avoids opaque weights: centrality first,
            # communication support second, stable motion third, ID last.
            candidates.append((
                robot.position.distance_to(observation_center),
                -robot_neighbor_count,
                speed_std,
                robot.robot_id,
                robot,
                direction,
        ...
        if not candidates:
            if not self.invalid_reference_recorded:
                self.invalid_reference_recorded = True
                self._lifecycle_row(
                    timestamp,
                    "SEARCHING",
                    activation_reason="invalid_anchor_reference",
                )
            if ANCHOR_SHADOW_VERBOSE:
                print("[AnchorShadow] invalid_anchor_reference")
            return

        _, _, _, _, anchor, direction = min(candidates, key=lambda item: item[:4])
        yaw, duration, displacement, _ = direction
        self.state = (
            AnchorJunctionState.PROVISIONAL_ANCHOR.value
            if PROVISIONAL_CONFIRMATION_ENABLED else "ELECTED"
        )
        self.anchor_robot_id = anchor.robot_id
        self.fixed_position = anchor.position.copy()
        self.reference_yaw_rad = yaw
        self.reference_history_duration_s = duration
        self.reference_displacement = displacement
        self.anchor_connected_before_election = anchor.connected_to_base
        self.anchor_neighbors_before_election = len(anchor.comm_neighbors)
        self._lifecycle_row(
            timestamp,
            self.state,
            election_reason=(
                "local_lateral_expansion_dwell; participant selected by "
                "centrality, communication degree, motion stability, robot ID"
            ),
        )

        anchor.role = "JUNCTION_ANCHOR"
        anchor.is_branch_leader = False
        anchor.anchor_shadow_fixed_position = self.fixed_position.copy()
        anchor.position = self.fixed_position.copy()
        ...
        if PROVISIONAL_CONFIRMATION_ENABLED:
            # The simulator-only ray caster may see map geometry and the world
            # pose. Only the copied local angle/range arrays cross into the
            # algorithm-visible confirmation state machine below.
            self.pointcloud_confirmation.begin_provisional(
                timestamp=timestamp,
                anchor_id=anchor.robot_id,
                local_reference_yaw_deg=math.degrees(yaw),
            )
            self.pointcloud_scan = simulate_polygon_lidar(
                polygon_points=cross_points,
                anchor_xy=(self.fixed_position.x, self.fixed_position.y),
                anchor_reference_yaw_deg=math.degrees(yaw),
                # This is a virtual-sensor range, not an opening-detector
                # threshold. Reuse the map's existing corridor-width scale so
                # no new detector constant is introduced in this integration.
                max_range_world_units=corridor_width,
                range_input_unit=ANCHOR_SHADOW_RANGE_INPUT_UNIT,
            )
            detector_angles, detector_ranges = self.pointcloud_scan.detector_input()
            decision = self.pointcloud_confirmation.evaluate(
                timestamp=timestamp,
                angles_deg=detector_angles,
                ranges=detector_ranges,
            )
            self._lifecycle_row(
                timestamp,
                self.pointcloud_confirmation.state.value,
                activation_reason=decision.reason,
            )
            if not decision.is_junction:
                self.state = AnchorJunctionState.REJECTED.value
                print(
                    f"[PointCloudConfirmation] REJECTED robot={anchor.robot_id}, "
                    f"openings={decision.opening_count}"
                )
                self.release(
                    observed,
                    timestamp,
                    "pointcloud_topological_degree_le_2_release",
                )
                return

            self.state = AnchorJunctionState.CONFIRMED_ANCHOR.value

        # This existing validator is deliberately created only after Point
        # Cloud confirmation when the new integration is enabled.
        self.validator = RelativeSwarmBranchValidator(
            temporal_window_s=4.0,
            minimum_observations=5,
            confidence_level=0.95,
            minimum_cohort_size=2,
            neighbor_edge_policy="reciprocal",
            maximum_neighbor_bearing_gap_deg=20.0,
        )
        self.state = (
            AnchorJunctionState.CONFIRMED_ANCHOR.value
            if PROVISIONAL_CONFIRMATION_ENABLED else "ACTIVE"
        )
        self.activation_timestamp = timestamp
        self.last_sample_timestamp = float("-inf")
        self._lifecycle_row(
            timestamp,
            self.state,
            activation_reason=(
                "pointcloud_topological_degree_ge_3_then_"
                "valid_pre_election_temporal_reference_and_fixed_pose"
                if PROVISIONAL_CONFIRMATION_ENABLED else
                "valid_pre_election_temporal_reference_and_fixed_pose"
            ),
        )
        print(
            f"[AnchorShadow] {self.state} robot={anchor.robot_id}, "
            f"yaw={math.degrees(yaw):.2f}deg, "
            f"expansion={self.evidence_expansion_ratio:.2f}"
        )

    def _exclusion_reason(self, robot) -> str:
        if robot.robot_id == self.anchor_robot_id:
            return "anchor_self"
        if robot.is_branch_leader:
            return "leader"
        if robot.role != "NORMAL":
            role_names = {
                ...
                "RELAY": "relay",
                "TRUNK_RELAY": "relay",
                "JUNCTION_GUARD": "guard",
                "FRONTIER_SHEPHERD": "shepherd",
                "PRE_SHEPHERD": "shepherd",
                "SHEPHERD": "shepherd",
            }
            return role_names.get(robot.role, f"role_{robot.role.lower()}")
        if not robot.connected_to_base:
            return "communication_disconnected"
        return ""

    def _sample(self, robots, timestamp: float) -> None:
        """Build the only data boundary visible to the existing validator."""
        if self.reference_yaw_rad is None or self.fixed_position is None:
            return
        observed_robots = [
            robot for robot in robots if not self._exclusion_reason(robot)
        ]
        observed_ids = {robot.robot_id for robot in observed_robots}
        observations = []
        reported_edges = set()
        for robot in observed_robots:
            offset = robot.position - self.fixed_position
            bearing = math.degrees(
                math.atan2(offset.y, offset.x) - self.reference_yaw_rad
            )
            bearing = (bearing + 180.0) % 360.0 - 180.0
            neighbor_ids = tuple(sorted(
                peer.robot_id for peer in robot.comm_neighbors
                if getattr(peer, "robot_id", None) in observed_ids
            ))
            for neighbor_id in neighbor_ids:
                reported_edges.add((robot.robot_id, neighbor_id))
            observation = RobotObservation(
                timestamp=timestamp,
                robot_id=robot.robot_id,
                anchor_range_m=offset.length(),
            ...
            self.observation_rows.append({
                "timestamp": timestamp,
                "anchor_robot_id": self.anchor_robot_id,
                "robot_id": robot.robot_id,
                "range": offset.length(),
                "bearing_deg": bearing,
        ...
        asymmetric = sorted(
            (first, second) for first, second in reported_edges
            if (second, first) not in reported_edges
        )
        self.neighbor_diagnostic_rows.append({
            "timestamp": timestamp,
            "non_reciprocal_edge_count": len(asymmetric),
            "non_reciprocal_pairs": json.dumps(asymmetric),
        })
        result = self.validator.update(observations)
        self.latest_result = result
        self.latest_observations = tuple(observations)
        self.observation_frame_count += 1
        count = len(result.branch_candidates)
        self.maximum_detected_branch_count = max(
            self.maximum_detected_branch_count, count
        )
        if self.previous_branch_count is not None and count != self.previous_branch_count:
            self.branch_count_change_count += 1
        self.previous_branch_count = count
        self.branch_count_history.append((timestamp, count))
        self.motion_count_history.append({
            "timestamp": timestamp,
            "progressing": len(result.progressing_robot_ids),
            "non_progressing": len(result.non_progressing_robot_ids),
            "returning": len(result.returning_robot_ids),
            "insufficient": len(result.insufficient_robot_ids),
        })

        component_by_robot = {
            robot_id: component_id
            for component_id, component in enumerate(result.progressing_components)
            for robot_id in component
        }
        cohort_by_robot = {
            robot_id: candidate.cohort_id
            for candidate in result.branch_candidates
            for robot_id in candidate.member_robot_ids
        }
        diagnostic_by_robot = {
            item.robot_id: item for item in result.robot_diagnostics
        }
        for trend in result.trends:
            diagnostic = diagnostic_by_robot[trend.robot_id]
            self.trend_rows.append({
                "timestamp": timestamp,
                "robot_id": trend.robot_id,
                "motion_state": trend.state,
                "observation_count": trend.observation_count,
                "radial_slope": trend.radial_slope_mps,
                "slope_standard_error": trend.slope_standard_error_mps,
                "ci_low": trend.slope_ci_low_mps,
                "ci_high": trend.slope_ci_high_mps,
                "residual_rmse": trend.residual_rmse_m,
                "representative_bearing_deg": trend.representative_bearing_deg,
                "graph_component_id": component_by_robot.get(trend.robot_id, ""),
                "cohort_id": cohort_by_robot.get(trend.robot_id, ""),
                "exclusion_reason": diagnostic.exclusion_reason or "",
                "range_input_unit": ANCHOR_SHADOW_RANGE_INPUT_UNIT,
            })
        for robot in robots:
            reason = self._exclusion_reason(robot)
            if not reason:
                continue
            self.trend_rows.append({
                "timestamp": timestamp,
                "robot_id": robot.robot_id,
                "motion_state": "excluded",
                "observation_count": 0,
                "radial_slope": "",
        ...
        common = {
            "timestamp": timestamp,
            "detected_branch_count": count,
            "rejected_component_count": len(result.rejected_progressing_components),
            "rejected_subcohort_count": len(result.rejected_bearing_subcohorts),
            "bearing_incompatible_edge_count": incompatible_count,
        }
        if result.branch_candidates:
            for candidate in result.branch_candidates:
                self.branch_rows.append({
                    **common,
                    "cohort_id": candidate.cohort_id,
                    "robot_count": candidate.robot_count,
                    "estimated_direction_deg": candidate.estimated_direction_deg,
                    "circular_spread_deg": candidate.circular_bearing_spread_deg,
                    "mean_resultant_length": candidate.mean_resultant_length,
                    "mean_radial_slope": candidate.mean_radial_slope_mps,
                    "minimum_ci_low": candidate.min_slope_ci_low_mps,
                })
        else:
            self.branch_rows.append({
                **common,
        ...
        if (
            OPENING_SWARM_DIAGNOSTICS_ENABLED
            and self.pointcloud_confirmation.decision is not None
        ):
            # Shadow-only information boundary: geometry, global poses, phase,
            # Branch labels, and expected directions are not forwarded.  The
            # ID graph below is copied only from the existing comm_neighbors.
            openings = tuple(
                OpeningInterval(
                    opening_id=index,
                    start_deg=float(opening["start_angle"]),
                    end_deg=float(opening["end_angle"]),
                    center_deg=float(opening["center_angle"]),
                    width_deg=float(opening["width_deg"]),
                    confidence=(
                        None if "confidence" not in opening
                        else float(opening["confidence"])
                    ),
                )
                for index, opening in enumerate(
                    self.pointcloud_confirmation.decision.openings
                )
            )
            robot_inputs = tuple(
                RobotTrendInput(
                    ...
                    representative_bearing_deg=item.representative_bearing_deg,
                    radial_slope=item.radial_slope_mps,
                    ci_low=item.slope_ci_low_mps,
                    existing_cohort_id=item.final_cohort_id,
                )
                for item in result.robot_diagnostics
            )
            cohort_inputs = tuple(
                CohortInput(
                    cohort_id=item.cohort_id,
                    member_robot_ids=item.member_robot_ids,
                    circular_mean_deg=item.estimated_direction_deg,
                    circular_spread_deg=item.circular_bearing_spread_deg,
                    mean_resultant_length=item.mean_resultant_length,
                    mean_radial_slope=item.mean_radial_slope_mps,
                    minimum_ci_low=item.min_slope_ci_low_mps,
                )
                for item in result.branch_candidates
            )
            neighbor_graph = {
                robot.robot_id: tuple(
                    peer.robot_id for peer in robot.comm_neighbors
                    if hasattr(peer, "robot_id")
                )
                for robot in robots
            }
            self.opening_swarm_diagnostics.add_sample(
                timestamp=timestamp,
                openings=openings,
                robots=robot_inputs,
                cohorts=cohort_inputs,
                anchor_id=self.anchor_robot_id,
                neighbor_graph=neighbor_graph,
            )

    def update(self, robots, timestamp: float, dt: float) -> None:
        """Advance lifecycle and sampling from simulation time only."""
        if not ANCHOR_SHADOW_ENABLED or self.state == "RELEASED":
            return
        if not self.lifecycle_rows:
            self._lifecycle_row(timestamp, self.state)
            if PROVISIONAL_CONFIRMATION_ENABLED:
                self.pointcloud_confirmation.observe_no_anchor(
                    timestamp=timestamp
                )
        self._update_position_histories(robots, timestamp)
        if self.state in {"SEARCHING", AnchorJunctionState.NO_ANCHOR.value}:
            observed = self._local_expansion_evidence(robots, dt)
            if (
                observed
                and self.evidence_expansion_dwell
                >= JUNCTION_EXPANSION_DWELL_TIME
            ):
                self._elect(observed, timestamp)
        ...
        anchor = next(
            (robot for robot in robots if robot.robot_id == self.anchor_robot_id),
            None,
        )
        if anchor is None:
            self.release(robots, timestamp, "anchor_robot_missing")
            return
        drift = anchor.position.distance_to(self.fixed_position)
        self.maximum_position_drift = max(self.maximum_position_drift, drift)
        nearby_robots = [
            robot for robot in robots
            if robot is not anchor
            and robot.position.distance_to(anchor.position) <= SMOOTHING_LENGTH
        ]
        nearby_contact_count = sum(
            robot.contact_detected for robot in nearby_robots
        )
        self.maximum_nearby_contact_count = max(
            self.maximum_nearby_contact_count,
            nearby_contact_count,
        )
        nearby_mean_speed = sum(
            robot.observed_velocity.length() for robot in nearby_robots
        ) / max(len(nearby_robots), 1)
        self.anchor_diagnostic_rows.append({
            "timestamp": timestamp,
            "anchor_robot_id": self.anchor_robot_id,
            "fixed_x": self.fixed_position.x,
            "fixed_y": self.fixed_position.y,
            "current_x": anchor.position.x,
            "current_y": anchor.position.y,
            "position_drift": drift,
            "maximum_position_drift": self.maximum_position_drift,
            "nearby_robot_count": len(nearby_robots),
            "nearby_contact_count": nearby_contact_count,
            "nearby_mean_speed": nearby_mean_speed,
            "connected_before_election": self.anchor_connected_before_election,
            "neighbors_before_election": self.anchor_neighbors_before_election,
            "connected_to_base": anchor.connected_to_base,
            "communication_neighbor_count": len(anchor.comm_neighbors),
        })
        if timestamp + EPSILON >= self.last_sample_timestamp + ANCHOR_SHADOW_SAMPLE_INTERVAL_S:
            self._sample(robots, timestamp)
            self.last_sample_timestamp = timestamp

    def release(self, robots, timestamp: float, reason: str) -> None:
        """Release the physical Anchor and permanently close its validator."""
        if not ANCHOR_SHADOW_ENABLED or self.state == "RELEASED":
            return
        anchor = next(
            (robot for robot in robots if robot.robot_id == self.anchor_robot_id),
        ...
        if anchor is not None:
            anchor.role = "NORMAL"
            anchor.anchor_shadow_fixed_position = None
            anchor.velocity.update(0.0, 0.0)
            anchor.acceleration.update(0.0, 0.0)
            anchor.filtered_acceleration.update(0.0, 0.0)
        self.state = "RELEASED"
        self.release_timestamp = timestamp
        self._lifecycle_row(timestamp, "RELEASED", release_reason=reason)
        if (
            PROVISIONAL_CONFIRMATION_ENABLED
            and self.pointcloud_confirmation.state
            != AnchorJunctionState.RELEASED
        ):
            self.pointcloud_confirmation.release(timestamp=timestamp, reason=reason)
        self.validator = None
        print(f"[AnchorShadow] RELEASED reason={reason}")
    ...
    def _save_plots(self, output_dir: Path) -> None:
        """Write diagnostic PNGs without adding a runtime plotting dependency."""
        try:
            import matplotlib
            matplotlib.use("Agg")
            import matplotlib.pyplot as plt
        except ImportError as error:
            print(f"[AnchorShadow] PNG skipped: {error}")
            return

        if self.branch_count_history:
            times, counts = zip(*self.branch_count_history)
            figure, axis = plt.subplots(figsize=(8, 3.5))
            axis.step(times, counts, where="post")
            axis.set(xlabel="simulation time [s]", ylabel="detected Branch count")
            axis.grid(alpha=0.3)
            figure.tight_layout()
            figure.savefig(output_dir / "branch_count_over_time.png", dpi=150)
            plt.close(figure)

        if self.motion_count_history:
            figure, axis = plt.subplots(figsize=(8, 4))
            times = [row["timestamp"] for row in self.motion_count_history]
            for key in ("progressing", "non_progressing", "returning", "insufficient"):
                axis.plot(times, [row[key] for row in self.motion_count_history], label=key)
            axis.set(xlabel="simulation time [s]", ylabel="robot count")
            axis.legend()
            axis.grid(alpha=0.3)
            figure.tight_layout()
            figure.savefig(output_dir / "motion_state_counts_over_time.png", dpi=150)
            plt.close(figure)
        ...
        if self.latest_observations:
            ...
            axis.scatter(
                [math.radians(item.anchor_bearing_deg) for item in self.latest_observations],
                [item.anchor_range_m for item in self.latest_observations],
                s=8,
                alpha=0.65,
            ...
            if self.latest_result is not None:
                ...
                for candidate in self.latest_result.branch_candidates:
                    angle = math.radians(candidate.estimated_direction_deg)
                    axis.plot([angle, angle], [0.0, maximum_range], linewidth=2)
            axis.set_title("Final Anchor-local observations (shadow inference)")
            figure.tight_layout()
            figure.savefig(output_dir / "final_anchor_local_polar.png", dpi=150)
            plt.close(figure)

        figure = plt.figure(figsize=(7, 4))
        axis = figure.add_subplot(111, projection="polar")
        shadow_directions = (
            [candidate.estimated_direction_deg for candidate in self.latest_result.branch_candidates]
            if self.latest_result is not None else []
        )
        existing_directions = []
        if self.reference_yaw_rad is not None:
            for descriptor in branch_descriptors_by_uid.values():
                direction = descriptor.local_outgoing_direction
                if direction.length_squared() <= EPSILON:
                    continue
                existing_directions.append(
                    (math.degrees(math.atan2(direction.y, direction.x) - self.reference_yaw_rad) + 180.0)
                    % 360.0 - 180.0
                )
        for index, direction in enumerate(shadow_directions):
            angle = math.radians(direction)
            axis.plot([angle, angle], [0.0, 1.0], color="tab:blue", label=(
                "shadow inference" if index == 0 else None
            ))
        for index, direction in enumerate(existing_directions):
            angle = math.radians(direction)
            axis.plot([angle, angle], [0.0, 1.0], "--", color="tab:orange", label=(
                "existing detector (evaluation only)" if index == 0 else None
            ))
        axis.set_title("Existing vs shadow directions")
        if shadow_directions or existing_directions:
            axis.legend(loc="upper right", bbox_to_anchor=(1.35, 1.15))
        figure.tight_layout()
        figure.savefig(output_dir / "existing_vs_shadow_directions.png", dpi=150)
        plt.close(figure)

    def save_outputs(self, robots) -> None:
        """Persist shadow-only inference and evaluation-separated summaries."""
        if not ANCHOR_SHADOW_ENABLED or self.outputs_saved:
            return
        self.outputs_saved = True
        output_dir = ANCHOR_SHADOW_OUTPUT_DIR
        output_dir.mkdir(parents=True, exist_ok=True)
        self._write_csv(output_dir / "anchor_lifecycle.csv", self.lifecycle_rows, [
            "timestamp", "state", "anchor_robot_id", "election_reason", "fixed_x",
            "fixed_y", "reference_yaw_deg", "reference_direction_source",
            "reference_history_duration_s", "reference_displacement",
            "activation_reason", "release_reason", "range_input_unit",
        ])
        self._write_csv(output_dir / "anchor_relative_observations.csv", self.observation_rows, [
            "timestamp", "anchor_robot_id", "robot_id", "range", "bearing_deg",
            "neighbor_ids", "range_input_unit",
        ])
        self._write_csv(output_dir / "anchor_shadow_trends.csv", self.trend_rows, [
            "timestamp", "robot_id", "motion_state", "observation_count",
            "radial_slope", "slope_standard_error", "ci_low", "ci_high",
            "residual_rmse", "representative_bearing_deg", "graph_component_id",
            "cohort_id", "exclusion_reason", "range_input_unit",
        ])
        self._write_csv(output_dir / "anchor_shadow_branches.csv", self.branch_rows, [
            "timestamp", "detected_branch_count", "cohort_id", "robot_count",
            "estimated_direction_deg", "circular_spread_deg", "mean_resultant_length",
            "mean_radial_slope", "minimum_ci_low", "rejected_component_count",
            "rejected_subcohort_count", "bearing_incompatible_edge_count",
        ])
        self._write_csv(output_dir / "anchor_shadow_anchor_diagnostics.csv", self.anchor_diagnostic_rows, [
            "timestamp", "anchor_robot_id", "fixed_x", "fixed_y", "current_x",
            "current_y", "position_drift", "maximum_position_drift",
            "nearby_robot_count", "nearby_contact_count", "nearby_mean_speed",
            "connected_before_election", "neighbors_before_election",
            "connected_to_base", "communication_neighbor_count",
        ])
        self._write_csv(output_dir / "anchor_shadow_neighbor_diagnostics.csv", self.neighbor_diagnostic_rows, [
            "timestamp", "non_reciprocal_edge_count", "non_reciprocal_pairs",
        ])
        existing_count = len(discovered_branch_uids())
        shadow_count = (
            len(self.latest_result.branch_candidates)
            if self.latest_result is not None else 0
        )
        summary = [{
            "anchor_elected": self.anchor_robot_id is not None,
            "anchor_robot_id": "" if self.anchor_robot_id is None else self.anchor_robot_id,
            "activation_timestamp": "" if self.activation_timestamp is None else self.activation_timestamp,
            "release_timestamp": "" if self.release_timestamp is None else self.release_timestamp,
            "observation_frame_count": self.observation_frame_count,
            "maximum_detected_branch_count": self.maximum_detected_branch_count,
            "final_detected_branch_count": shadow_count,
            "branch_count_change_count": self.branch_count_change_count,
            "existing_detector_final_branch_count": existing_count,
            "shadow_detector_final_branch_count": shadow_count,
            "count_match": existing_count == shadow_count,
            "maximum_anchor_position_drift": self.maximum_position_drift,
            "anchor_connected_before_election": self.anchor_connected_before_election,
            "anchor_connected_at_end": (
                "" if self.anchor_robot_id is None else next(
                    (
            ...
            "maximum_nearby_contact_count": self.maximum_nearby_contact_count,
            "existing_done_reached": phase == SimulationPhase.DONE,
            "shadow_enabled": ANCHOR_SHADOW_ENABLED,
            "control_unchanged": True,
            "range_input_unit": ANCHOR_SHADOW_RANGE_INPUT_UNIT,
        }]
        self._write_csv(output_dir / "anchor_shadow_summary.csv", summary, list(summary[0]))
        self._save_plots(output_dir)
        print(f"[AnchorShadow] outputs saved: {output_dir.resolve()}")

        if PROVISIONAL_CONFIRMATION_ENABLED:
            comparison_rows = []
            decision = self.pointcloud_confirmation.decision
            if decision is not None:
                comparison_rows.extend({
                    "timestamp": decision.timestamp,
                    "source": "pointcloud_opening",
                    "candidate_id": index,
                    "direction_deg": opening["center_angle"],
                    "robot_count": "",
                    "circular_spread_deg": "",
                } for index, opening in enumerate(decision.openings))
            swarm_directions = []
            if self.latest_result is not None:
                comparison_timestamp = (
                    self.branch_rows[-1]["timestamp"] if self.branch_rows
                    else simulation_time
                )
                for candidate in self.latest_result.branch_candidates:
                    swarm_directions.append(candidate.estimated_direction_deg)
                    comparison_rows.append({
                        "timestamp": comparison_timestamp,
                        "source": "relative_swarm_candidate",
                        "candidate_id": candidate.cohort_id,
                        "direction_deg": candidate.estimated_direction_deg,
                        "robot_count": candidate.robot_count,
                        "circular_spread_deg": (
                            candidate.circular_bearing_spread_deg
                        ),
                    })
            save_confirmation_artifacts(
                output_dir=PROVISIONAL_CONFIRMATION_OUTPUT_DIR,
                confirmation=self.pointcloud_confirmation,
                scan=self.pointcloud_scan,
                anchor_position_drift=self.maximum_position_drift,
                swarm_directions_deg=swarm_directions,
                swarm_candidate_rows=comparison_rows,
            )
            print(
                "[PointCloudConfirmation] outputs saved: "
                f"{PROVISIONAL_CONFIRMATION_OUTPUT_DIR.resolve()}"
            )
            if OPENING_SWARM_DIAGNOSTICS_ENABLED:
                self.opening_swarm_diagnostics.save(
                    OPENING_SWARM_DIAGNOSTICS_OUTPUT_DIR
                )
                print(
                    "[OpeningSwarmDiagnostics] outputs saved: "
                    f"{OPENING_SWARM_DIAGNOSTICS_OUTPUT_DIR.resolve()}"
                )


anchor_shadow_manager = AnchorShadowManager()


def anchor_shadow_release_for_final_return(robots) -> None:
    """Release only the physical Anchor; no inference result is consulted."""
    anchor_shadow_manager.release(
        robots,
        simulation_time,
        "existing_final_return_started",
    )


def draw_anchor_shadow_overlay(surface) -> None:
    """Render shadow-only reference and inferred cohort directions."""
    manager = anchor_shadow_manager
    if (
        not ANCHOR_SHADOW_ENABLED
        or manager.state not in {
            "ACTIVE",
            AnchorJunctionState.PROVISIONAL_ANCHOR.value,
            AnchorJunctionState.CONFIRMED_ANCHOR.value,
        }
        or manager.fixed_position is None
        or manager.reference_yaw_rad is None
    ):
        return
    origin = manager.fixed_position
    reference = origin + pygame.Vector2(
        math.cos(manager.reference_yaw_rad),
        math.sin(manager.reference_yaw_rad),
    ) * ANCHOR_SHADOW_REFERENCE_LINE_LENGTH
    pygame.draw.line(surface, ANCHOR_SHADOW_REFERENCE_COLOR, origin, reference, 3)
    if manager.pointcloud_confirmation.decision is not None:
        for opening in manager.pointcloud_confirmation.decision.openings:
            angle = manager.reference_yaw_rad + math.radians(
                opening["center_angle"]
            )
            endpoint = origin + pygame.Vector2(
                math.cos(angle), math.sin(angle)
            ) * ANCHOR_SHADOW_COHORT_LINE_LENGTH
            pygame.draw.line(
                surface,
                POINTCLOUD_OPENING_COLOR,
                origin,
                endpoint,
                3,
            )
    if manager.latest_result is not None:
        for candidate in manager.latest_result.branch_candidates:
            angle = manager.reference_yaw_rad + math.radians(
                candidate.estimated_direction_deg
            )
            endpoint = origin + pygame.Vector2(
                math.cos(angle), math.sin(angle)
            ) * ANCHOR_SHADOW_COHORT_LINE_LENGTH
...
def anchor_shadow_hud_lines() -> list[str]:
    """Return presentation-only state; it is never read by SPH control."""
    if not ANCHOR_SHADOW_ENABLED:
        return []
    result = anchor_shadow_manager.latest_result
    lines = [
        "SHADOW ONLY — CONTROL UNCHANGED",
        (
            f"Anchor shadow={anchor_shadow_manager.state} "
            f"id={anchor_shadow_manager.anchor_robot_id if anchor_shadow_manager.anchor_robot_id is not None else '-'}"
        ),
        (
            "Shadow branches=0 | P=0 NP=0 R=0 I=0"
            if result is None else
            f"Shadow branches={len(result.branch_candidates)} | "
            f"P={len(result.progressing_robot_ids)} "
            f"NP={len(result.non_progressing_robot_ids)} "
            f"R={len(result.returning_robot_ids)} "
            f"I={len(result.insufficient_robot_ids)}"
        ),
    ]
    if PROVISIONAL_CONFIRMATION_ENABLED:
        decision = anchor_shadow_manager.pointcloud_confirmation.decision
        lines.insert(1, (
            "PointCloud confirmation=NOT_EVALUATED"
            if decision is None else
            f"PointCloud openings={decision.opening_count} | "
            f"decision={'CONFIRMED' if decision.is_junction else 'REJECTED'}"
        ))
    return lines
...
def begin_final_return(robots):
    ...
    anchor_shadow_release_for_final_return(robots)
    release_transient_roles_for_final_return(robots)
    relay_slots = []
    relay_motion_scale = 1.0
...
while running:
    ...
    if not paused:
        ...
        anchor_shadow_manager.update(robots, simulation_time, frame_dt)
    ...
    draw_anchor_shadow_overlay(screen)
    ...
    hud_lines.extend(anchor_shadow_hud_lines())
...
anchor_shadow_manager.save_outputs(robots)
pygame.quit()
sys.exit()
