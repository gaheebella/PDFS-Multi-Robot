...
from junction_detection.integration.handoff_gt_frame_isolation_diagnostics import (
    HandoffGtFrameIsolationDiagnostics,
)
...
# This independent sink consumes the first failed RIGHT snapshot only.  Its GT
# fixture frame is evaluation-only and no value computed by it is returned to
# motion, role assignment, or the production handoff resolver.
GT_FRAME_DIAGNOSTICS_ENABLED = (
    os.environ.get("SPH_DFS_GT_FRAME_DIAGNOSTICS", "0") == "1"
)
GT_FRAME_DIAGNOSTICS_OUTPUT_DIR = Path(
    os.environ.get(
        "SPH_DFS_GT_FRAME_DIAGNOSTICS_OUTPUT_DIR",
        "/tmp/pdfs_gt_frame_handoff",
    )
)
gt_frame_diagnostics = HandoffGtFrameIsolationDiagnostics()
...
def _record_gt_frame_isolation(
    frontiers,
    descriptor: BranchDescriptor,
    ...
    """Shadow-replay the first failed RIGHT snapshot without control effects."""
    if (
        not GT_FRAME_DIAGNOSTICS_ENABLED
        or gt_frame_diagnostics.captured
        or descriptor.fixture_key != "RIGHT"
        or resolved_depth is not None
        or descriptor.observed_mouth_position is None
    ):
        return
    production_t, production_n = descriptor_local_basis(descriptor)
    origin = descriptor.observed_mouth_position
    frontier_rows = []
    for robot in frontiers:
        if robot.frontier_local_lateral is None:
            raise AssertionError("retained RIGHT frontier is missing its locked slot")
        ...
        frontier_rows.append({
            ...
            "actual_local_depth": actual_axial,
            "actual_local_lateral": actual_lateral,
            "lateral_slot": (
                frontier_line_lateral_center + robot.frontier_local_lateral
            ),
        })

    # Current cross fixture GT is legal only in this diagnostic callback.  It
    # is never written back into descriptor/controller state.
    gt_origin = (float(center_x + half_width), float(center_y))
    gt_tangent = (1.0, 0.0)
    gt_normal = (0.0, 1.0)
    collision_pixel_radius = max(1, int(round(ROBOT_RADIUS)))
    environment_state = {
        "fixture": "single_cross",
        "corridor_top": float(center_y - half_width),
        "corridor_bottom": float(center_y + half_width),
        "right_mouth_x": float(center_x + half_width),
        "right_terminal_x": float(center_x + half_width + right_length),
        "corridor_width": float(corridor_width),
        "right_length": float(right_length),
        "collision_pixel_radius": collision_pixel_radius,
        "collision_test": "is_walkable(position, ROBOT_RADIUS)",
    }
    fixture_center_bounds = {
        "center_top": float(
            center_y - half_width + collision_pixel_radius - 0.5
        ),
        "center_bottom": float(
            center_y + half_width - collision_pixel_radius + 0.5
        ),
        "center_terminal": float(
            ...
            - collision_pixel_radius + 0.5
        ),
    }
    gt_frame_diagnostics.capture_and_replay(
        ...
        production_origin=(origin.x, origin.y),
        production_tangent=(production_t.x, production_t.y),
        production_normal=(production_n.x, production_n.y),
        gt_origin=gt_origin,
        ...
        observed_width=descriptor.observed_width,
        observed_physical_width=descriptor.observed_physical_width,
        ...
        physical_width_source=descriptor.physical_width_source,
        environment_state=environment_state,
        fixture_center_bounds=fixture_center_bounds,
        walkable=lambda point: is_walkable(
            pygame.Vector2(point[0], point[1]), ROBOT_RADIUS
        ),
        production_attempted_rows=attempted_rows,
    )
    print(
        "[GTFrameIsolation] captured first failed RIGHT snapshot "
        f"frame={globals().get('headless_frame_count', 0)} "
        f"t={simulation_time:.6f} frontiers={len(frontier_rows)}"
    )
...
if GT_FRAME_DIAGNOSTICS_ENABLED:
    gt_frame_diagnostics.save(GT_FRAME_DIAGNOSTICS_OUTPUT_DIR)
pygame.quit()
sys.exit()
