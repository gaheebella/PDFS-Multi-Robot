"""EXP-045 Local Worst-Wall-Range Estimation from Moving LiDAR.

This self-contained copy preserves EXP-044's active manual-W detector and
anchor semantics.  A local angle/range-only W estimator runs in shadow mode;
noise-free scan ground truth is evaluation-only and never feeds threshold,
confirmation, anchor control, or motion.
"""
...
EXPERIMENT_NAME = "EXP-045 Local Worst-Wall-Range Estimation from Moving LiDAR"
DEFAULT_OUTPUT = ROOT / "lidar_junction_detection_local_w_estimator_output"
FROZEN_PARAMETERS = {
...
class DetectorExperimentConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.w_source != "manual":
            raise ValueError("EXP-045 active w_source must remain manual")
...
class AdaptiveRenderer(BaseRenderer):
    ...
    def _draw_w_shadow_panel(
        ...
        pygame = self.pygame
        panel = pygame.Surface((390, 220), pygame.SRCALPHA)
        ...
        estimate = snapshot.local_w_estimate
        lines = (
            "W SOURCE ACTIVE = MANUAL",
            f"active W = {config.worst_wall_range:.3f}",
            "LOCAL W ESTIMATOR = SHADOW (NO FEEDBACK)",
            "input = " + (
                "SMOOTHED measured"
                if config.w_estimator_use_smoothed
                else "RAW measured"
            ),
            f"far threshold = {estimate.far_support_threshold:.3f}",
            f"far runs={len(estimate.far_runs)} candidates={len(estimate.candidates)}",
            f"W_est max={fmt(estimate.w_est_max)}",
            f"W_est median={fmt(estimate.w_est_median)}",
            f"W_est p90={fmt(estimate.w_est_p90)}",
            f"history median={fmt(snapshot.w_history_median)} p90={fmt(snapshot.w_history_p90)}",
            f"W_GT={fmt(snapshot.w_gt_eval_only)} [EVAL ONLY]",
            f"errors max/median/p90={fmt(snapshot.w_error_max)}/{fmt(snapshot.w_error_median)}/{fmt(snapshot.w_error_p90)}",
            f"W_EST_AVAILABLE = {estimate.available}",
            "GREEN O = SHADOW CANDIDATE; RED X = GT EVAL ONLY",
        )
        for index, line in enumerate(lines):
            color = COLORS["text"]
            if "SHADOW" in line or "GREEN" in line:
                color = COLORS["clear"]
            if "GT" in line or "RED" in line:
    ...
    def draw(self, session: AdaptiveSession, paused: bool) -> None:
        ...
        self.text(
            f"EXP-045 Local W Shadow Estimator — {detector_title}",
            (18, 15),
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=("manual",),
        default="manual",
        help="active detector source; EXP-045 intentionally permits manual only",
    )
    parser.add_argument(
